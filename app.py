...
from Conver_Agent.render.pyrender.smpl_render import SMPLRender, SMPLX2020Render
...
model.to(device)
model.lm.to(torch.bfloat16)

other_tools.load_checkpoints(model.vq_model_face, cfg.TRAIN.PRETRAINED_VAE_FACE,
                             cfg.vq.emage.params.e_name)
other_tools.load_checkpoints(model.vq_model_upper, cfg.TRAIN.PRETRAINED_VAE_UPPER,
                             cfg.vq.emage.params.e_name)
other_tools.load_checkpoints(model.vq_model_hands, cfg.TRAIN.PRETRAINED_VAE_HAND,
                             cfg.vq.emage.params.e_name)
other_tools.load_checkpoints(model.vq_model_lower, cfg.TRAIN.PRETRAINED_VAE_LOWER,
                             cfg.vq.emage.params.e_name)
other_tools.load_checkpoints(model.global_motion, cfg.TRAIN.PRETRAINED_VAE_GLOBAL,
                             cfg.vq.emage.params.e_name)
...
def emage_token_to_string_v2(face_token, hand_token, lower_token, upper_token, lengths):
    motion_string = []
...
def render_motion_exp(rec_pose, rec_trans, rec_exps, method='fast'):
    fname = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(
        time.time())) + str(np.random.randint(10000, 99999))
    video_fname = fname + '.mp4'
    feats_fname = fname + '.npy'
    ...
    if len(rec_pose.shape) == 4:
        rec_pose = rec_pose[0]

    rec_pose = rec_pose.reshape(-1,55,3)

    shape = [768, 768]
    render = SMPLX2020Render(cfg.RENDER.SMPLX2020_MODEL_PATH)

    r = RRR.from_rotvec(np.array([np.pi, 0.0, 0.0]))
    root_vec = RRR.from_rotvec(np.array(rec_pose[:, 0]))
    root_vec_tranfromed = np.matmul(r.as_matrix().reshape(1, 3, 3), root_vec.as_matrix().reshape(-1, 3, 3))  ## noticed
    root_vec_tranfromed_aa = RRR.from_matrix(root_vec_tranfromed).as_rotvec()
    rec_pose[:, 0] = root_vec_tranfromed_aa
    rec_trans = np.matmul(r.as_matrix().reshape(3, 3), rec_trans.T).T
    vid = []
    rec_trans = rec_trans - rec_trans[0]
    aroot = rec_trans[[0]]
    # rec_trans[:,1] = -rec_trans[:,1]
    # aroot[:, 1] = -aroot[:, 1]
    params = dict(pred_shape=np.zeros([1, 300]),
                  pred_root=aroot,
                  pred_pose=rec_pose.reshape(-1,55*3),
                  pred_trans=rec_trans,
                  pred_exps=rec_exps)

    render.init_renderer([shape[0], shape[1], 3], params)

    for i in range(rec_pose.shape[0]):
        renderImg = render.render(i)
        vid.append(renderImg)

    out = np.stack(vid, axis=0)
    output_gif_path = output_mp4_path[:-4] + '.gif'
    imageio.mimwrite(output_gif_path, out, duration=33.3)
    out_video = mp.VideoFileClip(output_gif_path)
    out_video.write_videofile(output_mp4_path)
    del out, render
...
def load_motion(motion_uploaded, method):
    ...
    motion_token_string = model.lm.motion_token_to_string(motion_token, [motion_token.shape[1]])[0]
    model.lm.emage_token_to_string_v2(motion_token, [motion_token.shape[1]])[0]
...
def bot(history, motion_uploaded, data_stored, method):
    ...
    out_rec_pose = outputs["rec_pose"][:out_lengths].detach().cpu().numpy()
    out_rec_trans = outputs["rec_trans"][:out_lengths].detach().cpu().numpy()
    out_rec_exps = outputs["rec_exps"][:out_lengths].detach().cpu().numpy()
    out_texts = outputs["texts"][0]

    output_mp4_path, video_fname, output_npy_path, joints_fname = render_motion_exp(
        out_rec_pose,
        out_rec_trans,
        out_rec_exps,
        method)
    ...
    if '<Upper_Placeholder>' in out_texts or '<Lower_Placeholder>' in out_texts:
        response = [
...
with gr.Blocks(css=customCSS) as demo:
    ...
    Init_chatbot = chat_instruct.value[:
                                       1] + t2m_examples.value[:
                                                               3] + m2t_examples.value[:3] + t2t_examples.value[
                                                                                             :2] + chat_instruct.value[
                                                                                                   -7:]
