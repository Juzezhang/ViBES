...
from multimodal_tokenizers.metrics.base import BaseMetrics
from multimodal_tokenizers.config import get_obj_from_str
# from torch import Tensor, nn

# Remove all the metric imports since they're handled by BaseMetrics
...
class BaseModel(LightningModule):
    ...
    def on_validation_epoch_end(self):
        # Log steps and losses
        dico = self.step_log_dict()
        # Log losses (only if they were updated)
        if hasattr(self, "_losses") and 'losses_train' in self._losses:
            train_count = float(self._losses['losses_train'].count)
            if train_count > 0:
                dico.update(self.loss_log_dict('train'))
        if hasattr(self, "_losses") and 'losses_val' in self._losses:
            val_count = float(self._losses['losses_val'].count)
            if val_count > 0:
    ...
    def configure_metrics(self):
        ...
        self.metrics = BaseMetrics(**self.hparams)
