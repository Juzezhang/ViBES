...
def plot_3d_motion_lower(args, figsize=(10, 10), fps=120, radius=4):
    matplotlib.use('Agg')
    ...
    #     [4, 5], [6, 7]
    # ]
    smpl_kinetic_chain = [[0, 2, 4, 6, 8], [0, 1, 3, 5, 7]]
    # smpl_kinetic_chain = [
    #     [0, 11], [0, 1, 2, 4],
    #     [5, 7], [8, 10]
    # ]
    ...
    limits = 1000 if nb_joints == 9 else 2
    ...
    def update(index):
        ...
        fig = plt.figure(figsize=(480 / 96., 320 / 96.),
                         dpi=96) if nb_joints == 9 else plt.figure(
                             figsize=(10, 10), dpi=96)
        # fig.tight_layout()
        if title is not None:
            wraped_title = '\n'.join(wrap(title, 40))
...
def draw_to_batch_lower(smpl_joints_batch, title_batch=None, outname=None):

    batch_size = len(smpl_joints_batch)
    out = []
    for i in range(batch_size):
        out.append(
            plot_3d_motion_lower([
                smpl_joints_batch[i], None,
                title_batch[i] if title_batch is not None else None
            ]))
        if outname is not None:
