...
class MLM(nn.Module):
    ...
    def forward(self, 
        ...
        for i in range(len(inputs)):
            if "qwen" in self.language_model.config._name_or_path.lower():
                # Qwen chat format
                full_sequences.append(f"<|im_start|>user\n{inputs[i]}<|im_end|>\n<|im_start|>assistant\n{outputs[i]}<|im_end|>")
            elif "llama" in self.language_model.config._name_or_path.lower():
                # Llama instruction format
                full_sequences.append(f"<s>[INST] {inputs[i]} [/INST] {outputs[i]}</s>")
            else:
                # Default format for other models
                full_sequences.append(f"{inputs[i]} ### {outputs[i]}")
    ...
    def generate_direct(self,
        ...
        for i, (prompt, output) in enumerate(zip(input, outputs_string)):
            ...
            # For Llama models
                ...
                outputs_string[i] = assistant_resp
            # For other models
            elif "###" in prompt and "###" in output:
                assistant_resp = output.split("###")[-1].strip()
                outputs_string[i] = assistant_resp
