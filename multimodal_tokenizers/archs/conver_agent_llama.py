...
class ConverAgentLlama(nn.Module):
    ...
    def forward(self, 
        ...
        face_strings, hand_strings, upper_strings, lower_strings, motion_string = self.compositional_motion_token_to_string(
            face_tokens, hand_tokens, lower_tokens, upper_tokens, motion_lengths)
        ...
        for i in range(len(inputs)):
                full_sequences.append(f"<s>[INST] {inputs[i]} [/INST] {outputs[i]}</s>")


        # Tokenize with Llama 3 specific settings
        target_inputs = self.tokenizer(full_sequences,
                              padding='max_length',
        ...
        for i, sequence in enumerate(full_sequences):
            # Find the position of [/INST] in the tokenized sequence
            input_tokens = self.tokenizer.encode(f"<s>[INST] {inputs[i]} [/INST]", add_special_tokens=False)
        ...
        input_ids = target_inputs.input_ids.to(face_tokens.device)
        attention_mask = target_inputs.attention_mask.to(face_tokens.device)
    ...
    def generate_direct(self,
        ...
        formatted_inputs = []
        for inp in input:
            if "<s>[INST]" not in inp:
                formatted_inputs.append(f"<s>[INST] {inp} [/INST]")
            else:
        ...
        # Default generation parameters for Llama and other models
        ...
        for i, (prompt, output) in enumerate(zip(input, outputs_string)):
            # For Llama models
            assistant_resp = output.split("[/INST]")[-1].strip()
            outputs_string[i] = assistant_resp


        # Convert strings back to tokens for different body parts
        face_tokens, hand_tokens, lower_tokens, upper_tokens, cleaned_text = self.compositional_string_to_token_seprate(outputs_string)
    ...
    def emage_token_to_string_v3(self, face_token: Tensor, hand_token: Tensor, lower_token: Tensor, upper_token: Tensor, lengths: List[int]):
        ...
        for i in range(len(lengths)):
            ...
            motion_string_tmp = '[Mo0]'
            for j in range(lengths[i]):
                motion_string_tmp = motion_string_tmp  + ''.join(f'[Lo{int(lower_list[j])}]') + ''.join(f'[Up{int(upper_list[j])}]')
            motion_string_tmp += '[Mo1]'
            motion_string.append(motion_string_tmp)
    ...
    def get_middle_str_emage(self, content, startStr, endStr):
        ...
        #     endIndex = content.index(endStr)
        # except:
        #     return '[Mo0][Fa0][Ha0][Lo0][Up0][Mo1]'
        try:
            startIndex = content.index(startStr)
        except:
            # return '[Mo0][Fa0][Ha0][Lo0][Up0][Mo1]'
            return '[Mo0][Lo0][Up0][Mo1]'
    ...
    def get_middle_str_emage_v2(self, content, startStr, endStr):
        ...
        try:
            ...
            return startStr + '[Fa0][Ha0][Lo0][Up0]' + endStr
