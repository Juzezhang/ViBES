...
class MixedDatasetCB(data.Dataset):
    ...
    def _load_candor(self, config):
        ...
        """
        # Skip loading CANDOR for test split
        if self.split == "test":
        ...
        ##################  CANDOR  ##################
        # Define paths
        audio_token_dir = pjoin(data_root, code_path_audio)
        face_token_dir = pjoin(data_root, code_path)
        structure_path = pjoin(data_root, 'candor_structure.json')
        
        # Load conversation structure
        with open(structure_path, 'r') as f:
            candor_structure = json.load(f)
            
        # Initialize empty tasks if instructions file is not provided
        self.tasks_candor = []
        
        # Determine the instructions path based on the stage, only if instructions_file is provided
        if instructions_file is not None:
            ...
            # Load instructions and tasks for CANDOR
            self.instructions_candor = json.load(open(instructions_path, 'r'))
            for task in self.instructions_candor.keys():
                for subtask in self.instructions_candor[task].keys():
                    self.tasks_candor.append(self.instructions_candor[task][subtask])
        else:
            # If no instructions file, create a default task
            print("No instructions file provided for CANDOR, using default task")
            self.tasks_candor = [{
                "class": "default",
                "task": "audio to motion",
                "instruction": "Generate facial expressions from audio",
                "prompt": "Generate facial expressions from the audio",
        ...
        for index, conv_id in enumerate(track(conversation_ids, description=f"Loading CANDOR {self.split}")):
            try:
                # Get speaker IDs for this conversation
                speaker_files = candor_structure[conv_id]
                ...
                for speaker_file in speaker_files:
                    ...
                    audio_path = pjoin(audio_token_dir, conv_id, f"{speaker_id}.npy")
                    if not os.path.exists(audio_path):
                        print(f"Audio tokens not found: {audio_path}")
                        continue
                    
                    # Path to face tokens
                    face_path = pjoin(face_token_dir, conv_id, f"{speaker_id}.npy")
                    ...
                    # Process into chunks based on motion segments
                    audio_chunks, face_chunks = self._process_candor_tokens(audio_tokens, face_tokens)
                    
                    for i, (audio_chunk, face_chunk) in enumerate(zip(audio_chunks, face_chunks)):
                        # Create entries for each task
                        for task in self.tasks_candor:
                            new_name = f'candor_{conv_id}_{speaker_id}_{i}_{task["class"]}'
                            self.data_dict_candor[new_name] = {
                                'face_token': face_chunk,
                                'hand_token': None,  # CANDOR doesn't have hand tokens
                                'lower_token': None,  # CANDOR doesn't have lower body tokens
                                'upper_token': None,  # CANDOR doesn't have upper body tokens
                                'audio': audio_chunk,
                                'text': None,  # CANDOR may not have aligned text
                                'tasks': task,
                                'emotion_label': None,
                            }
                            self.metadata_candor.append(new_name)
        ...
        print(f"Loaded {len(self.metadata_candor)} CANDOR samples for {self.split} split")
        
        # Save to cache after processing
        self._save_to_cache(cache_path, self.data_dict_candor, self.metadata_candor, "CANDOR")
        
    def _process_candor_tokens(self, audio_tokens, face_tokens):
        """
        Process CANDOR audio and face tokens into chunks for training.
        
        Args:
            audio_tokens: Audio tokens array
            face_tokens: Face tokens array
            
        Returns:
            Tuple of (audio_chunks, face_chunks) lists
        """
        # Calculate frame rates
        audio_token_fps = self.audio_token_fps  # From class initialization
        face_token_fps = audio_token_fps / 2  # Face tokens typically at half the rate of audio
        
        # Define chunk sizes
        audio_chunk_size = int(self.unit_length * audio_token_fps)
        face_chunk_size = int(self.unit_length * face_token_fps)
        
        # Calculate overlap (50% overlap for training)
        audio_stride = audio_chunk_size // 2 if self.split == 'train' else audio_chunk_size
        face_stride = face_chunk_size // 2 if self.split == 'train' else face_chunk_size
        ...
        for i in range(0, len(audio_tokens) - audio_chunk_size, audio_stride):
            # Extract audio chunk
            audio_chunk = audio_tokens[i:i + audio_chunk_size]
            
            # Calculate corresponding indices for face tokens
            face_start = i // 2
            face_end = face_start + face_chunk_size
            
            # Skip if we don't have enough face tokens
            if face_end > len(face_tokens):
                continue
    ...
    def _get_cache_path(self, config, data_root):
        ...
        if dataset_type == "BEAT2":
            ...
            config_str = f"conversational_{dataset_type}_{self.split}_{self.stage}_{self.test_length}_{code_path}_{code_path_audio}_{additional_data}_{speakers_str}_{vary_length_str}_{debug_str}"
            ...
            # Format boolean values more descriptively
            debug_str = "Debug" if self.debug else "NoDebug"
            config_str = f"conversational_{dataset_type}_{self.split}_{self.stage}_{self.test_length}_{code_path}_{code_path_audio}_{debug_str}"
            ...
            config_str = f"conversational_{dataset_type}_{self.split}_{self.stage}_{trainset_str}_{vary_length_str}_{debug_str}"
        
        elif dataset_type == "CANDOR":
            # Get CANDOR-specific configuration
            code_path = config.get("code_path", "")
            code_path_audio = config.get("code_path_audio", "")
            structure_file = config.get("structure_file", "candor_structure.json")
            
            # Format boolean values more descriptively
            vary_length_str = "VaryLength" if self.vary_length else "NoVaryLength"
            debug_str = "Debug" if self.debug else "NoDebug"
            
            # Create a unique cache identifier for CANDOR
            config_str = f"conversational_{dataset_type}_{self.split}_{self.stage}_{self.test_length}_{code_path}_{code_path_audio}_{structure_file}_{vary_length_str}_{debug_str}"
