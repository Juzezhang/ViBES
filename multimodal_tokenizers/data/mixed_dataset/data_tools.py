...
# Create joint masks as global variables
def _create_joint_mask(ori_joints, tar_joints):
    """Helper function to create a joint mask."""
    ori_joint_list = joints_list[ori_joints]
    tar_joint_list = joints_list[tar_joints]
    
    joint_mask = np.zeros(len(list(ori_joint_list.keys())) * 3)
    
    for joint_name in tar_joint_list:
        joint_mask[ori_joint_list[joint_name][1] - ori_joint_list[joint_name][0]:ori_joint_list[joint_name][1]] = 1
        
    return joint_mask

# Create joint masks as global variables
def _create_joint_mask_6D(ori_joints, tar_joints):
    """Helper function to create a joint mask."""
    ori_joint_list = joints_list[ori_joints]
    tar_joint_list = joints_list[tar_joints]
    
    joint_mask = np.zeros(len(list(ori_joint_list.keys())) * 6)
...
# Define common joint list keys as constants
BEAT_SMPLX_JOINTS = 'beat_smplx_joints'
BEAT_SMPLX_FULL = 'beat_smplx_full'
BEAT_SMPLX_FACE = 'beat_smplx_face'
BEAT_SMPLX_FACE_HEAD = 'beat_smplx_face_head'
...
# Pre-compute common joint masks
ORI_JOINTS = BEAT_SMPLX_JOINTS
ORI_JOINTS_6D = BEAT_SMPLX_JOINTS_6D
JOINT_MASK_FACE = _create_joint_mask(ORI_JOINTS, BEAT_SMPLX_FACE)
JOINT_MASK_FACE_HEAD = _create_joint_mask(ORI_JOINTS, BEAT_SMPLX_FACE_HEAD)
