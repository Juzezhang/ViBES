...
class UpperVQDataset(data.Dataset):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f"[UpperVQDataset] Loaded {len(self.metadata)} samples")
    ...
    def _get_cache_path(self, dataset_path, dataset_type):
        ...
        config_str = f"{dataset_type}_upper_{self.split}_vq"
    ...
    def _save_to_cache(self, cache_path, data_dict, metadata, dataset_name):
        ...
        try:
            ...
            print(f"Saving {dataset_name} upper dataset to cache: {cache_path}")
            ...
            print(f"Successfully saved {dataset_name} upper cache to {cache_path}")
            
        except Exception as e:
            print(f"Error saving {dataset_name} upper cache: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _load_from_cache(self, cache_path, dataset_name):
        ...
        if not os.path.exists(cache_path):
            print(f"Upper cache file not found: {cache_path}")
            return None, None
        
        try:
            print(f"Loading {dataset_name} upper dataset from cache: {cache_path}")
            ...
            print(f"Successfully loaded {dataset_name} upper cache from {cache_path}")
            return data_dict, metadata
            
        except Exception as e:
            print(f"Error loading {dataset_name} upper cache: {str(e)}")
            import traceback
            traceback.print_exc()
            return None, None
    ...
    def _load_beat2(self, config):
        ...
        for index, file_name in tqdm(self.selected_file.iterrows()):
            ...
            round_seconds_skeleton = tar_pose_upper_6d.shape[0] // pose_fps_beat2
    ...
    def _load_amass(self, config):
        ...
        for index, file_name in tqdm(enumerate(id_list_amass)):
            try:
                ...
                # Calculate time segments
                round_seconds_skeleton = tar_pose_upper_6d.shape[0] // pose_fps_amass
                if round_seconds_skeleton == 0:
                    round_seconds_skeleton = 1
