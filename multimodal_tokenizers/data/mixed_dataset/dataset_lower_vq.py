...
class LowerVQDataset(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f"[LowerVQDataset] Loaded {len(self.metadata)} samples")
    ...
    def _get_cache_path(self, dataset_path, dataset_type):
        ...
        config_str = f"{dataset_type}_lower_{self.split}_vq"
    ...
    def _save_to_cache(self, cache_path, data_dict, metadata, dataset_name):
        ...
        try:
            ...
            print(f"Saving {dataset_name} lower dataset to cache: {cache_path}")
            ...
            print(f"Successfully saved {dataset_name} lower cache to {cache_path}")
            
        except Exception as e:
            print(f"Error saving {dataset_name} lower cache: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _load_from_cache(self, cache_path, dataset_name):
        ...
        if not os.path.exists(cache_path):
            print(f"Lower cache file not found: {cache_path}")
            return None, None
        
        try:
            print(f"Loading {dataset_name} lower dataset from cache: {cache_path}")
            ...
            print(f"Successfully loaded {dataset_name} lower cache from {cache_path}")
            return data_dict, metadata
            
        except Exception as e:
            print(f"Error loading {dataset_name} lower cache: {str(e)}")
            import traceback
            traceback.print_exc()
            return None, None
