...
from customloss import (camera_fitting_loss, 
                        ...
                        camera_fitting_loss_3d_smplx,
                        body_fitting_loss_3d, 
                        body_fitting_loss_3d_smplx,
                        )
...
def guess_init_3d(model_joints, 
    ...
    gt_joints_smpl = ['RHip', 'LHip', 'RShoulder', 'LShoulder']
    
    gt_joints_smplx = ['R_Hip', 'L_Hip', 'R_Shoulder', 'L_Shoulder']
    gt_joints_ind = [config.JOINT_MAP_SMPLX2020[joint] for joint in gt_joints_smplx]
    
    if joints_category=="orig":
        joints_ind_category = [config.JOINT_MAP_SMPLX2020[joint] for joint in gt_joints_smplx]
    elif joints_category=="AMASS":
        joints_ind_category = [config.AMASS_JOINT_MAP[joint] for joint in gt_joints_smpl] 
    elif joints_category=="MMM":
        joints_ind_category = [config.MMM_JOINT_MAP[joint] for joint in gt_joints_smplx] 
    else:
...
class SMPLify3D():
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        ...
        smpl_output = self.smpl(global_orient=global_orient,
                                body_pose=body_pose,
                                betas=betas)
        model_joints = smpl_output.joints  ## shape (B, 127, 3) because of smplx model
        ...
        if self.use_lbfgs:
            ...
            # for i in range(self.num_iters):
                def closure():
                    ...
                    loss = body_fitting_loss_3d_smplx(body_pose, preserve_pose, betas, model_joints[:, self.smpl_index], camera_translation,
            ...
            ## TODO: Not implemented with smplx model
...
# SMPLIfy 3D
class SMPLifyX3D():
    """Implementation of SMPLifyX, use 3D joints with SMPL-X model."""
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        ...
        smpl_output = self.smpl(
            ...
            expression=torch.zeros(betas.shape[0], 100).to(self.device),
            jaw_pose=torch.zeros(betas.shape[0], 3).to(self.device),
            global_orient=global_orient,
            body_pose=body_pose[:, :21 * 3],
            left_hand_pose=torch.zeros(betas.shape[0], 15 * 3).to(self.device),
            right_hand_pose=torch.zeros(betas.shape[0], 15 * 3).to(self.device),
            leye_pose=torch.zeros(betas.shape[0], 3).to(self.device),
            reye_pose=torch.zeros(betas.shape[0], 3).to(self.device),
            )
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        if self.use_lbfgs:
            ...
            for i in range(10):
                def closure():
                    ...
                    loss = camera_fitting_loss_3d_smplx(model_joints, camera_translation,
                                                  init_cam_t, j3d, self.joints_category)
                    loss.backward()
                    return loss
            ...
            for i in range(20):
                ...
                loss = camera_fitting_loss_3d_smplx(model_joints[:, self.smpl_index], camera_translation,
                                              init_cam_t,  j3d[:, self.corr_index], self.joints_category)
                camera_optimizer.zero_grad()
                loss.backward()
                camera_optimizer.step()
        ...
        with torch.no_grad():
            ...
            final_loss = body_fitting_loss_3d_smplx(body_pose, preserve_pose, betas, model_joints[:, self.smpl_index], camera_translation,
                                              j3d[:, self.corr_index], self.pose_prior,
                                              joints3d_conf=conf_3d,
                                              joint_loss_weight=600.0,
                                              use_collision=self.use_collision, model_vertices=model_vertices, model_faces=self.model_faces,
