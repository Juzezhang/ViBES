...
def body_fitting_loss_3d(body_pose, preserve_pose,
    ...
    total_loss = joint3d_loss + pose_prior_loss.unsqueeze(-1) + angle_prior_loss.unsqueeze(-1) + shape_prior_loss.unsqueeze(-1) + collision_loss + pose_preserve_loss.unsqueeze(-1)
...
def body_fitting_loss_3d_smplx(body_pose, preserve_pose,
                         betas, model_joints, camera_translation,
                         j3d, pose_prior,
                         joints3d_conf,
                         sigma=100, pose_prior_weight=4.78*1.5,
...
# #####--- get camera fitting loss -----
def camera_fitting_loss_3d_smplx(model_joints, camera_t, camera_t_est,
                           j3d, joints_category="orig", depth_loss_weight=100.0):
    """
    Loss function for camera optimization.
    """
    ...
    gt_joints_amass = ['RHip', 'LHip', 'RShoulder', 'LShoulder']
    ...
    if joints_category=="orig":
        select_joints_ind = [config.JOINT_MAP_SMPLX2020[joint] for joint in gt_joints_smplx]
    elif joints_category=="AMASS":
        select_joints_ind = [config.AMASS_JOINT_MAP[joint] for joint in gt_joints_amass]
    elif joints_category=="MMM":
        select_joints_ind = [config.MMM_JOINT_MAP[joint] for joint in gt_joints_smplx]
    else:
        print("NO SUCH JOINTS CATEGORY!")
