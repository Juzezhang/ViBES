"""Body model wrappers vendored from GVHMR for self-contained rendering."""

import torch
import torch.nn as nn
import smplx


_kwargs_disable_smplx = {
    "create_body_pose": False,
    "create_betas": False,
    "create_global_orient": False,
    "create_transl": False,
    "create_left_hand_pose": False,
    "create_right_hand_pose": False,
    "create_expression": False,
    "create_jaw_pose": False,
    "create_leye_pose": False,
    "create_reye_pose": False,
}

_kwargs_disable_smplh = {
    "create_body_pose": False,
    "create_betas": False,
    "create_global_orient": False,
    "create_transl": False,
    "create_left_hand_pose": False,
    "create_right_hand_pose": False,
}


class BodyModelSMPLX(nn.Module):
    """Batch-inference wrapper around smplx.create for SMPLX models."""

    def __init__(self, model_path, **kwargs):
        super().__init__()
        kwargs.update(_kwargs_disable_smplx)
        self.bm = smplx.create(model_path=model_path, **kwargs)
        self.faces = self.bm.faces
        self.hand_pose_dim = (
            self.bm.num_pca_comps if self.bm.use_pca else 3 * self.bm.NUM_HAND_JOINTS
        )

        # Pre-compute joint regressor for fast skeleton computation
        shapedirs = self.bm.shapedirs  # (V, 3, num_betas)
        J_regressor = self.bm.J_regressor[:22, :]  # (22, V)
        v_template = self.bm.v_template  # (V, 3)
        J_template = J_regressor @ v_template  # (22, 3)
        J_shapedirs = torch.einsum("jv, vcd -> jcd", J_regressor, shapedirs)  # (22, 3, num_betas)
        self.register_buffer("J_template", J_template, False)
        self.register_buffer("J_shapedirs", J_shapedirs, False)

    def forward(
        self,
        betas=None,
        global_orient=None,
        transl=None,
        body_pose=None,
        left_hand_pose=None,
        right_hand_pose=None,
        expression=None,
        jaw_pose=None,
        leye_pose=None,
        reye_pose=None,
        **kwargs,
    ):
        device, dtype = self.bm.shapedirs.device, self.bm.shapedirs.dtype

        model_vars = [
            betas, global_orient, body_pose, transl, expression,
            left_hand_pose, right_hand_pose, jaw_pose, leye_pose, reye_pose,
        ]
        batch_size = 1
        for var in model_vars:
            if var is not None:
                batch_size = max(batch_size, len(var))

        if global_orient is None:
            global_orient = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        if body_pose is None:
            body_pose = (
                torch.zeros(3 * self.bm.NUM_BODY_JOINTS, device=device, dtype=dtype)[None]
                .expand(batch_size, -1).contiguous()
            )
        if left_hand_pose is None:
            left_hand_pose = (
                torch.zeros(self.hand_pose_dim, device=device, dtype=dtype)[None]
                .expand(batch_size, -1).contiguous()
            )
        if right_hand_pose is None:
            right_hand_pose = (
                torch.zeros(self.hand_pose_dim, device=device, dtype=dtype)[None]
                .expand(batch_size, -1).contiguous()
            )
        if jaw_pose is None:
            jaw_pose = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        if leye_pose is None:
            leye_pose = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        if reye_pose is None:
            reye_pose = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        if expression is None:
            expression = torch.zeros(
                [batch_size, self.bm.num_expression_coeffs], dtype=dtype, device=device
            )
        if betas is None:
            betas = torch.zeros([batch_size, self.bm.num_betas], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)

        return self.bm(
            betas=betas,
            global_orient=global_orient,
            body_pose=body_pose,
            left_hand_pose=left_hand_pose,
            right_hand_pose=right_hand_pose,
            transl=transl,
            expression=expression,
            jaw_pose=jaw_pose,
            leye_pose=leye_pose,
            reye_pose=reye_pose,
            **kwargs,
        )

    def get_skeleton(self, betas):
        """betas: (*, 10) -> skeleton_beta: (*, 22, 3)"""
        return self.J_template + torch.einsum("...d, jcd -> ...jc", betas, self.J_shapedirs)

    def forward_bfc(self, **kwargs):
        """Wrap (B, F, C) to (B*F, C) and unwrap back."""
        for k in kwargs:
            assert len(kwargs[k].shape) == 3
        B, F = kwargs["body_pose"].shape[:2]
        smplx_out = self.forward(**{k: v.reshape(B * F, -1) for k, v in kwargs.items()})
        smplx_out.vertices = smplx_out.vertices.reshape(B, F, -1, 3)
        smplx_out.joints = smplx_out.joints.reshape(B, F, -1, 3)
        return smplx_out


class BodyModelSMPLH(nn.Module):
    """Batch-inference wrapper around smplx.create for SMPL/SMPLH models."""

    def __init__(self, model_path, **kwargs):
        super().__init__()
        kwargs.update(_kwargs_disable_smplh)
        self.bm = smplx.create(model_path=model_path, **kwargs)
        self.faces = self.bm.faces
        self.is_smpl = kwargs.get("model_type", "smpl") == "smpl"
        if not self.is_smpl:
            self.hand_pose_dim = (
                self.bm.num_pca_comps if self.bm.use_pca else 3 * self.bm.NUM_HAND_JOINTS
            )

        shapedirs = self.bm.shapedirs
        J_regressor = self.bm.J_regressor[:22, :]
        v_template = self.bm.v_template
        J_template = J_regressor @ v_template
        J_shapedirs = torch.einsum("jv, vcd -> jcd", J_regressor, shapedirs)
        self.register_buffer("J_template", J_template, False)
        self.register_buffer("J_shapedirs", J_shapedirs, False)

    def forward(
        self,
        betas=None,
        global_orient=None,
        transl=None,
        body_pose=None,
        left_hand_pose=None,
        right_hand_pose=None,
        **kwargs,
    ):
        device, dtype = self.bm.shapedirs.device, self.bm.shapedirs.dtype

        model_vars = [betas, global_orient, body_pose, transl, left_hand_pose, right_hand_pose]
        batch_size = 1
        for var in model_vars:
            if var is not None:
                batch_size = max(batch_size, len(var))

        if global_orient is None:
            global_orient = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        if body_pose is None:
            body_pose = (
                torch.zeros(3 * self.bm.NUM_BODY_JOINTS, device=device, dtype=dtype)[None]
                .expand(batch_size, -1).contiguous()
            )
        if not self.is_smpl:
            if left_hand_pose is None:
                left_hand_pose = (
                    torch.zeros(self.hand_pose_dim, device=device, dtype=dtype)[None]
                    .expand(batch_size, -1).contiguous()
                )
            if right_hand_pose is None:
                right_hand_pose = (
                    torch.zeros(self.hand_pose_dim, device=device, dtype=dtype)[None]
                    .expand(batch_size, -1).contiguous()
                )
        if betas is None:
            betas = torch.zeros([batch_size, self.bm.num_betas], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)

        return self.bm(
            betas=betas,
            global_orient=global_orient,
            body_pose=body_pose,
            left_hand_pose=left_hand_pose,
            right_hand_pose=right_hand_pose,
            transl=transl,
            **kwargs,
        )

    def get_skeleton(self, betas):
        """betas: (*, 10) -> skeleton_beta: (*, 22, 3)"""
        return self.J_template + torch.einsum("...d, jcd -> ...jc", betas, self.J_shapedirs)
