...
class CopyModelCodeCallback(TrainerCallback):
    """Copy modeling_chatglm.py to checkpoint dirs so they can be loaded with AutoModel."""
    def on_save(self, args, state, control, **kwargs):
        checkpoint_dir = os.path.join(args.output_dir, f"checkpoint-{state.global_step}")
        src = os.path.join(os.path.dirname(__file__), "..", "vibes", "modeling_chatglm.py")
        if os.path.isdir(checkpoint_dir) and os.path.isfile(src):
            shutil.copy2(src, checkpoint_dir)
...
def tokenized_data_collator(batch):
    ...
    for item in batch:
        # Dynamic style token replacement: randomly sample from style_pool
        style_pool = item.get("style_pool", [])
        if style_pool:
            # Find style region: modality_2=True positions before first modality_1=True position
            mod1_positions = [i for i, v in enumerate(item["modality_masks_1"]) if v]
            mod2_positions = [i for i, v in enumerate(item["modality_masks_2"]) if v]

            if mod1_positions and mod2_positions:
                first_audio = mod1_positions[0]
                style_positions = [p for p in mod2_positions if p < first_audio]

                if style_positions and len(style_pool) >= len(style_positions):
                    max_start = len(style_pool) - len(style_positions)
                    start = random.randint(0, max_start)
                    new_style = style_pool[start : start + len(style_positions)]

                    # Replace input_ids at style positions (mutable copy)
                    input_ids = list(item["input_ids"])
                    for pos, new_id in zip(style_positions, new_style):
                        input_ids[pos] = new_id
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        ...
        callbacks=[CopyModelCodeCallback()],
    )
