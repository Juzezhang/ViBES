...
class MLM(nn.Module):
    ...
    def forward(self, 
        ...
        if self.lm_type == 'encdec':
            return self.forward_encdec(texts, face_tokens, hand_tokens, upper_tokens, lower_tokens, audio_tokens, motion_lengths, audio_lengths, tasks, emotion_label)
        elif self.lm_type == 'dec':
            return self.forward_dec(texts, face_tokens, hand_tokens, upper_tokens, lower_tokens, audio_tokens, motion_lengths, audio_lengths, tasks, emotion_label)
        else:
            raise NotImplementedError("Only encdec or dec supported")
    ...
    def forward_encdec(
        ...
        # Tensor to string
        face_strings, hand_strings, upper_strings, lower_strings, motion_string = self.compositional_motion_token_to_string(face_token, hand_token, lower_token, upper_token, lengths)
        ...
        if condition == 'text':
            ...
            inputs, outputs = self.template_fulfill(tasks, lengths, audio_length, face_strings, hand_strings, upper_strings, lower_strings, motion_string, audio_strings, texts, combine_strings, emotion_label)
    ...
    def forward_dec(
        ...
        # For decoder-only models, we concatenate input and output with a separator
        full_sequences = []
        ...
        for i, sequence in enumerate(full_sequences):
            input_length = len(self.tokenizer(inputs[i], return_tensors="pt").input_ids[0])
            labels[i, :input_length] = -100  # Don't compute loss for input portion
    ...
    def generate_conditional(self,
        ...
        if task in ["t2m", "m2m", "pred", "inbetween", "a2m", "at2m"]:
            ...
            combine_strings_list = [''] * batch_size if combine_strings is None else combine_strings
            ...
            inputs, outputs = self.template_fulfill(
                ...
                combine_strings_list, emotion_strings
            )
    ...
    def generate_conditional_legacy(self,
                              texts=None,
                              text_timestamp=None,
                              face_tokens=None,
                              hand_tokens=None,
                              ...
                              tasks=None,
                              emotion_label=None):
        """Backward-compatible wrapper for the new generate_conditional method"""
        
        # Prepare body tokens dictionary
        body_tokens_dict = None
        if any(token is not None for token in [face_tokens, hand_tokens, upper_tokens, lower_tokens]):
            body_tokens_dict = {}
            if face_tokens is not None: body_tokens_dict['face'] = face_tokens
            if hand_tokens is not None: body_tokens_dict['hand'] = hand_tokens
            if upper_tokens is not None: body_tokens_dict['upper'] = upper_tokens
            if lower_tokens is not None: body_tokens_dict['lower'] = lower_tokens
        
        # Prepare audio data dictionary
        audio_data_dict = None
        if audio_tokens is not None or onset is not None or amplitude_envelope is not None:
            audio_data_dict = {}
            if audio_tokens is not None: audio_data_dict['tokens'] = audio_tokens
            if onset is not None: audio_data_dict['onset'] = onset
            if amplitude_envelope is not None: audio_data_dict['amplitude_envelope'] = amplitude_envelope
            if text_timestamp is not None: audio_data_dict['timestamps'] = text_timestamp
        
        # Prepare lengths dictionary
        lengths_dict = {}
        if lengths is not None: lengths_dict['motion'] = lengths
        if audio_lengths is not None: lengths_dict['audio'] = audio_lengths
        
        # Prepare context dictionary
        context_dict = {}
        if text_timestamp is not None: context_dict['text_timestamp'] = text_timestamp
        if combine_strings is not None: context_dict['combine_strings'] = combine_strings
        if emotion_label is not None: context_dict['emotion_label'] = emotion_label
        
        # Call new implementation
        result = self.generate_conditional(
            texts=texts,
            body_tokens=body_tokens_dict,
        ...
        # For backward compatibility, unpack the dictionary result
        if isinstance(result, dict) and all(k in result for k in ['face', 'hand', 'lower', 'upper']):
            return result['face'], result['hand'], result['lower'], result['upper'], result.get('text')
