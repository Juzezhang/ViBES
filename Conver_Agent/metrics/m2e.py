...
class M2EMetrics(Metric):
    ...
    def __init__(self,
        ...
        if self.cfg.model.params.task == 'm2e':
    ...
    def compute(self, sanity_flag):
        ...
        #                                              device=self.device)
        metrics[f"Bleu_1"] = torch.tensor(scores[f'bleu_1']['score'], device=self.device)
        metrics[f"Bleu_4"] = torch.tensor(scores[f'bleu_4']['score'], device=self.device)
