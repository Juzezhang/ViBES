...
import cv2
import utils_emage.media
import utils_emage.fast_render
        ...
        for root, dirs, files in os.walk(folder_path):
            for file in files:
                ...
                if file.endswith('.wav'):
                    ...
                    if base_name == '22_luqi_0_9_9' or base_name == "22_luqi_0_15_15" or base_name == '22_luqi_0_16_16' or base_name == '22_luqi_0_10_10':
                        continue
...
def load_checkpoints(model, save_path, load_name='model'):
    ...
    states = torch.load(save_path, weights_only=True, map_location=torch.device('cpu'))
...
def load_checkpoints_tokenizer(model, save_path, load_name='model'):
    # states = torch.load(save_path)
    # states = torch.load(save_path, weights_only=True, map_location=torch.device('cpu'))
    state_dict = torch.load(save_path, map_location=torch.device('cpu'))["state_dict"]
