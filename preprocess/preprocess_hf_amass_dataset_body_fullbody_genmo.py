...
"""
Preprocess AMASS dataset into question-answer sequences with fullbody_genmo motion modality,
organizing audio and fullbody_genmo tokens into fixed-size chunks and saving in
HuggingFace datasets format. Fullbody GENMO version with single body part.
...
- Fullbody GENMO tokens under data_root/TOKENS_AGENT_25/fullbody_genmo
...
- Processes question-answer pairs with fullbody_genmo motion supervision
- Fullbody_genmo tokens mapped to upper token vocabulary (<|upper_X|>)
- Audio FPS: 12.5, Motion FPS: 6.25 (6.25*1) for single fullbody part
- Group size: question (text OR audio) + answer text (with embedded audio) + 1 begin_of_motion + 13 fullbody_genmo motion tokens per group
- Token breakdown: 1 begin_of_motion + 13 motion tokens (fullbody_genmo mapped as upper)
- 3 modalities: text(0), audio(1), motion(2)
- Different system prompts for text vs audio input

Usage:
    python preprocess_hf_amass_dataset_body_fullbody_genmo.py \
        --data_root /path/to/AMASS_talking \
        --output_path ./processed_amass_fullbody_genmo_train \
        --split train
"""
...
def initialize_token_sets(tokenizer):
    ...
    """
    global UPPER_TOKEN_IDS

    # Initialize upper token IDs (fullbody_genmo tokens are mapped as upper)
    UPPER_TOKEN_IDS = set()
    for i in range(256):
        try:
    ...
    logging.info(f"Initialized token sets: {len(UPPER_TOKEN_IDS)} upper tokens (fullbody_genmo mapped as upper)")
...
def calculate_position_encoding_indices(modality_masks, modality_fps=None):
    ...
    Args:
        ...
        modality_fps: Dictionary mapping modality index to fps value
                     Default: {1: 12.5, 2: 6.25} (Fullbody GENMO version, motion=6.25fps=6.25*1)
...
def process_full_video(sequence_id, transcripts_question_dir, transcripts_answer_dir, audio_question_dir, fullbody_genmo_dir,
                       audio_fps=12.5, fullbody_genmo_fps=6.25):
    """
    ...
    Args:
        ...
        fullbody_genmo_dir: Directory containing fullbody GENMO tokens
        fullbody_genmo_fps: Fullbody GENMO token frame rate

    Returns:
        List of processed segments with text, audio, and fullbody_genmo tokens (mapped as upper)
    """
    processed_segments = []
    ...
    fullbody_genmo_token_file = os.path.join(fullbody_genmo_dir, f"{sequence_id}.npy")
    ...
    if not os.path.exists(fullbody_genmo_token_file):
        logging.warning(f"No fullbody_genmo tokens found for {sequence_id}, skipping")
        return []
    ...
    # Load fullbody_genmo tokens (mapped as upper)
    try:
        fullbody_genmo_data = np.load(fullbody_genmo_token_file, allow_pickle=True)
        if fullbody_genmo_data.ndim > 1:
            fullbody_genmo_data = fullbody_genmo_data[0]  # Take first element if nested

        # Calculate target duration based on fullbody_genmo tokens and FPS
        target_duration = len(fullbody_genmo_data) / fullbody_genmo_fps
        logging.info(f"Target duration based on fullbody_genmo tokens: {target_duration:.2f}s")

        # Calculate required number of fullbody_genmo tokens based on duration and FPS
        target_fullbody_genmo_length = int(target_duration * fullbody_genmo_fps)
        if len(fullbody_genmo_data) < target_fullbody_genmo_length:
            # Repeat the sequence to match target duration
            repeat_times = (target_fullbody_genmo_length + len(fullbody_genmo_data) - 1) // len(fullbody_genmo_data)
            fullbody_genmo_data_extended = np.tile(fullbody_genmo_data, repeat_times)[:target_fullbody_genmo_length]
            logging.info(f"Extended fullbody_genmo tokens from {len(fullbody_genmo_data)} to {target_fullbody_genmo_length} by repeating (duration: {target_duration:.2f}s)")
        elif len(fullbody_genmo_data) > target_fullbody_genmo_length:
            # Truncate to match target duration
            fullbody_genmo_data_extended = fullbody_genmo_data[:target_fullbody_genmo_length]
            logging.info(f"Truncated fullbody_genmo tokens from {len(fullbody_genmo_data)} to {target_fullbody_genmo_length} (duration: {target_duration:.2f}s)")
        else:
            fullbody_genmo_data_extended = fullbody_genmo_data
            logging.info(f"Fullbody_genmo tokens already match target duration: {len(fullbody_genmo_data)} tokens for {target_duration:.2f}s")

        # Create list of (token, timestamp) tuples based on FPS
        fullbody_genmo_tokens_ts = [(fullbody_genmo_data_extended[i], i / fullbody_genmo_fps) for i in range(len(fullbody_genmo_data_extended))]

        logging.info(f"Final fullbody_genmo tokens: {len(fullbody_genmo_data_extended)}, duration: {len(fullbody_genmo_data_extended) / fullbody_genmo_fps:.2f}s")

    except Exception as e:
        logging.error(f"Error loading fullbody_genmo tokens for {sequence_id}: {e}")
        return []

    # Build a single segment with all data
    start_time = 0.0
    end_time = target_duration
    segment_fullbody_genmo = fullbody_genmo_tokens_ts
    if transcripts_question and transcripts_answer:
        processed_segments.append({
            ...
            "end_time": end_time,
            "upper_tokens": [t for t, _ in segment_fullbody_genmo],  # fullbody_genmo mapped as upper
            ...
            "has_upper_file": os.path.exists(fullbody_genmo_token_file),
        })
...
def process_chunk_to_record(chunk, conv_id, tokenized_records, tokenizer):
    """
    Process a chunk of turns into a tokenized record with fullbody_genmo motion modality support.
    ...
    modality_masks = [
        ...
        chunk["modality_masks_2"],  # Fullbody_genmo motion tokens (modality 2, mapped as upper)
    ]
    ...
    tokenized_record = {
        ...
        "modality_masks_2": chunk["modality_masks_2"],  # True for fullbody_genmo motion tokens (mapped as upper)
...
def convert_to_huggingface_dataset(
    ...
    """
    Convert interleaved turns into a HuggingFace dataset with fullbody_genmo motion modality support.
    Following AMASS format: assistant-only, no system prompt, fullbody_genmo motion supervision.
    Packs multiple turns into sequences up to max_seq_length.
    Fullbody GENMO version: only fullbody_genmo tokens mapped as upper.
    ...
    Args:
        ...
        upper_segment_size: Number of fullbody_genmo tokens per group (default: 13, mapped as upper)
        limit_sequences: Limit number of sequences to process
        split: Dataset split name
    Returns:
        Dataset: The created HuggingFace dataset
    """
    logging.info(f"Converting to Hugging Face dataset (fullbody_genmo version) with tokenizer: {tokenizer.__class__.__name__}")
    ...
    for conv_idx, (conv_id, turns) in enumerate(conversations.items()):
        ...
        if len(turns) > 0:
            ...
            # Get the full motion token arrays directly (fullbody_genmo mapped as upper)
            full_upper_tokens = first_turn.get("upper_tokens", [])  # fullbody_genmo tokens mapped as upper
            ...
            # Calculate required audio groups count based on motion tokens length
            # Each audio group (26 tokens) corresponds to 13 fullbody_genmo tokens (mapped as upper)
            # So: required_audio_groups = len(full_upper_tokens) / 13
            required_audio_groups = len(full_upper_tokens) // 13
            ...
            if len(all_audio_groups) > required_audio_groups:
                ...
                logging.info(f"Truncated audio groups from {len(all_audio_groups)} to {required_audio_groups} to match motion tokens length ({len(full_upper_tokens)} fullbody_genmo tokens)")
            else:
                # Use all available audio groups if not enough
                audio_groups = all_audio_groups
                if len(audio_groups) < required_audio_groups:
                    logging.warning(f"Only {len(audio_groups)} audio groups available, but {required_audio_groups} needed for {len(full_upper_tokens)} fullbody_genmo tokens")

            # Use motion tokens as-is (no adjustment needed)
            adjusted_upper_tokens = full_upper_tokens

            # Determine supervision based on file existence rather than token values
            upper_has_real_data = first_turn.get("has_upper_file", False)

            # Now insert fullbody_genmo motion tokens (mapped as upper) after each audio group
            final_tokens = []
            final_labels = []
            final_modality_0 = []
            final_modality_1 = []
            ...
            for group_idx, group in enumerate(audio_groups):
                ...
                # Get fullbody_genmo tokens for this specific audio group (mapped as upper)
                upper_tokens_chunk = adjusted_upper_tokens[group_idx*13:(group_idx+1)*13] if group_idx*13 < len(adjusted_upper_tokens) else []
                if len(upper_tokens_chunk) < 13:
                    upper_tokens_chunk.extend([0] * (13 - len(upper_tokens_chunk)))

                # Create motion token sequence: 1 begin_of_motion + 13 fullbody_genmo tokens (as upper)
                motion_token_ids = []
                motion_labels = []
                motion_modality_0 = []
                motion_modality_1 = []
                ...
                motion_labels.append(begin_motion_token)
                ...
                # Process fullbody_genmo tokens sequentially (mapped as upper)
                for upper_idx in range(len(upper_tokens_chunk)):
                    ...
                    motion_labels.extend(upper_token_ids if upper_has_real_data else [-100] * len(upper_token_ids))  # Supervised if file exists
                    motion_modality_0.extend([False] * len(upper_token_ids))  # Not text
                    motion_modality_1.extend([False] * len(upper_token_ids))  # Not audio
                    motion_modality_2.extend([True] * len(upper_token_ids))   # Motion modality 2
            ...
            # Copy remaining TEXT content after the last audio+motion group,
            # but truncate at the next audio token (no orphan audio without motion).
            if current_pos < len(cleaned_answer_tokens):
                remaining_tokens = []
                for token_id in cleaned_answer_tokens[current_pos:]:
                    if 152353 <= token_id <= 168735:
                        break  # Stop before orphan audio tokens
                    remaining_tokens.append(token_id)
                if remaining_tokens:
                    final_tokens.extend(remaining_tokens)
                    for token_id in remaining_tokens:
                        final_labels.append(-100)
                        final_modality_0.append(True)
                        final_modality_1.append(False)
                        final_modality_2.append(False)
        ...
        if current_input_ids and len(current_input_ids) > len(system_prompt_tokens):
            ...
            process_chunk_to_record(tokenized_record, conv_id, tokenized_records, tokenizer)
    ...
    metadata = {
        ...
        "format_version": "3.0",
        "format_type": "user_assistant_with_system_and_fullbody_genmo_motion_modality",
        "text_format": "question (text OR audio) + answer text (with embedded audio) + 1 begin_of_motion + 13 fullbody_genmo motion tokens (mapped as upper) with motion supervision and precomputed position encoding indices",
        "position_encoding": "precomputed_indices_based_on_modality_fps",
        "modality_fps": {"1": 12.5, "2": 6.25},
        "supervision": "fullbody_genmo motion tokens (mapped as upper, supervised if available)",
        "tokenized": True,
        "max_seq_length": max_seq_length,
        "system_prompt": "text_and_audio_system_prompts_available",
        "assistant_prefix": "included_in_answer_text",
        "modality_masks": "masks_0 for text, masks_1 for audio, masks_2 for fullbody_genmo motion (mapped as upper)",
        "modality_supervision": "fullbody_genmo_as_upper",
        "source": "AMASS_question_answer_fullbody_genmo"
    }
...
def main():
    """Main function to preprocess AMASS dataset with fullbody_genmo tokens and question-answer format."""
    parser = argparse.ArgumentParser(description="Preprocess AMASS dataset with fullbody_genmo tokens and question-answer format")
    ...
    parser.add_argument("--fullbody_genmo_dir", type=str, default="TOKENS_AGENT_25/fullbody_genmo",
                       help="Directory containing fullbody GENMO token files (relative to data_root)")
    # Processing parameters
    parser.add_argument("--audio_fps", type=float, default=12.5,
                       help="Audio tokens per second")
    parser.add_argument("--fullbody_genmo_fps", type=float, default=6.25,
                       help="Fullbody GENMO tokens per second")
    ...
    logging.info("Starting preprocessing of AMASS dataset (fullbody_genmo version)")
    ...
    # Add special tokens for face, upper body, lower body, and hand modalities (for vocab compatibility)
    ...
    fullbody_genmo_dir = os.path.join(args.data_root, args.fullbody_genmo_dir)
    sequence_ids = []
    if not os.path.isdir(transcripts_answer_dir) or not os.path.isdir(transcripts_question_dir) or not os.path.isdir(audio_question_dir) or not os.path.isdir(fullbody_genmo_dir):
        logging.error("One or more required directories do not exist: transcripts_answer_dir, transcripts_question_dir, audio_question_dir, fullbody_genmo_dir")
        return
    ...
    for transcripts_answer_path in Path(transcripts_answer_dir).glob("*.txt"):
        ...
        fullbody_genmo_file = os.path.join(fullbody_genmo_dir, f"{sequence_id}.npy")
        transcripts_question_file = os.path.join(transcripts_question_dir, f"{sequence_id}.txt")
        audio_question_file = os.path.join(audio_question_dir, f"{sequence_id}.npy")
        if os.path.exists(fullbody_genmo_file) and os.path.exists(transcripts_question_file) and os.path.exists(audio_question_file):
            sequence_ids.append(sequence_id)
        else:
            logging.debug(f"Skipping {sequence_id}, missing required files (fullbody_genmo, transcripts_question, or audio_question)")
    ...
    for sequence_id in tqdm(sequence_ids, desc="Processing videos"):
        ...
        # Process full video into single segment
        processed_segments = process_full_video(
            sequence_id, transcripts_question_dir, transcripts_answer_dir, audio_question_dir, fullbody_genmo_dir, args.audio_fps, args.fullbody_genmo_fps
        )
        ...
        for seg in processed_segments:
            ...
            all_turns.append({
                ...
                "upper_tokens": seg["upper_tokens"],  # fullbody_genmo tokens mapped as upper
