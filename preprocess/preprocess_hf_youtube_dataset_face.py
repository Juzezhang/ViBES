...
based on TalkNet output, organizing audio and face motion tokens into fixed-size chunks,
and saving in HuggingFace datasets format with unified motion modality V3 format.
...
Sequence Filtering:
    - Requires speaking track, audio tokens, and face tokens
    - Sequences without face tokens are discarded
...
Usage:
    python preprocess_youtube_dataset_w_transcript_tokenized_varying_mot_A_only_encode_position_face_only_v3.py \
        --data_root /path/to/YouTube_Talking \
        --output_path ./processed_youtube_train_face_only_v3 \
        --split train
        
    python preprocess_youtube_dataset_w_transcript_tokenized_varying_mot_A_only_encode_position_face_only_v3.py \
        --data_root /path/to/YouTube_Talking \
        --output_path ./processed_youtube_test_face_only_v3 \
        --split test
"""
...
# Global precomputed token ID sets for efficient attention mask generation (not needed for face-only)
# All tokens participate in attention for face-only version
...
def process_transcript_segments(video_id, transcript_segments, audio_dir, face_dir,
    ...
    # Check for face tokens (required)
    has_face = os.path.exists(face_token_file)
    if not has_face:
    ...
    for seg_idx, segment in enumerate(transcript_segments):
        ...
        segment_face = get_tokens_for_timerange(face_tokens_ts, start_time, end_time)
        
        # Convert word data to word timestamps format
        word_timestamps = [(w['word'], w['start'], w['end']) for w in word_data]
        
        # Only process segments with content and face tokens
        if segment_audio and text and segment_face:
            ...
            logging.debug(f"Segment {seg_idx}: {start_time:.2f}s-{end_time:.2f}s, "
                         ...
                         f"face tokens: {len(segment_face)}, words: {len(word_data)}")
    
    return processed_segments

def split_into_chunks(processed_segments, audio_chunk_size=26, face_chunk_size=52):
    """
    Split processed segments into fixed-size chunks (Face-Only V3).
    
    Args:
        processed_segments: List of processed segments with text and tokens
        audio_chunk_size: Number of audio tokens per chunk (default: 26)
        face_chunk_size: Number of face tokens per chunk (default: 52)
    ...
    for segment in processed_segments:
        ...
        for chunk_idx in range(num_chunks):
            ...
            face_start = chunk_idx * face_chunk_size
            face_end = min((chunk_idx + 1) * face_chunk_size, len(face_tokens))
            chunk_face = face_tokens[face_start:face_end] if face_start < len(face_tokens) else []
            
            # Pad face if needed
            if len(chunk_face) < face_chunk_size:
                chunk_face = chunk_face + [0] * (face_chunk_size - len(chunk_face))
            ...
            all_chunks.append({
                ...
                # Pass file existence flag for supervision logic
                "has_face_file": segment.get("has_face_file", False)
            })
...
def convert_to_huggingface_dataset(
    ...
    """
    Convert interleaved turns into a HuggingFace dataset with unified motion modality V3 format (Face-Only).
    ...
    metadata = {
        ...
        "source": "YouTube_Talking_A_only_face_only_v3"
    }
...
def main():
    ...
    logging.info("Starting preprocessing of YouTube_Talking dataset (A-only face only V3 version)")
    ...
    for video_dir in os.listdir(transcript_dir):
        ...
        if os.path.isdir(video_path) and video_dir in selected_video_ids:
            ...
            face_file = os.path.join(face_dir, f"{video_dir}.npy")
            
            # Require speaking track, audio tokens, and face tokens
            has_face = os.path.exists(face_file)
            
            if os.path.exists(speaking_track_file) and os.path.exists(audio_file) and has_face:
                ...
                if not has_face:
                    missing_files.append("face tokens")
