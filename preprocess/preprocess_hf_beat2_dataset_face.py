...
"""
Preprocess BEAT2 dataset into A-only sequences (assistant-only) for Face-Only V3 version,
organizing audio and face motion tokens into fixed-size chunks with unified motion modality.

This script targets the BEAT2 dataset at
  /path/to/BEAT2/beat_english_v2.0.0
and assumes:
- Audio tokens under data_root/audios_token_glm
- Face tokens under data_root/TOKENS_AGENT_25/face
...
- Processes face motion tokens with unified modality
- Audio FPS: 12.5, Face Motion FPS: 25.0
- Group size: text + 26 audio + 1 begin_of_motion + 52 face motion tokens per group
- Token breakdown: 1 begin_of_motion + 52 face motion tokens
- 3 modalities: text(0), audio(1), motion(2)

Usage:
    python preprocess_beat2_dataset_w_transcript_tokenized_varying_mot_encode_position_face_only_v3.py \
        --data_root /path/to/BEAT2/beat_english_v2.0.0 \
        --output_path ./processed_beat2_train \
        --split train
"""
...
def calculate_position_encoding_indices(modality_masks, modality_fps=None):
    ...
    Args:
        ...
        modality_fps: Dictionary mapping modality index to fps value
                     Default: {1: 12.5, 2: 25.0} (Face-only V3, 3 modalities only, motion=25.0fps=face only)
...
def safe_token_to_string(token, modality):
    ...
    Args:
        ...
        modality: Type of token ("audio" or "face") - Face-Only V3
...
def process_full_video(video_id, transcript_text, audio_dir, face_dir,
                       audio_fps=12.5, face_fps=25.0):
    """
    Process transcript segments into format suitable for dataset creation (Face-Only V3).
    ...
    Returns:
        List of processed segments with text, audio, and face tokens only
    """
    processed_segments = []

    # Load audio and face motion tokens (Face-Only V3)
...
def interleave_groups_from_full_segment(segment, audio_group_size: int = 26, face_group_size: int = 52, audio_fps: float = 12.5):
    """
    From one full A-only segment, build interleaved groups following BEAT2 Face-Only V3 format:
    per group: text (words in time window), 26 audio tokens, 1 begin_of_motion token, 52 face motion tokens.
    """
...
def convert_to_huggingface_dataset(
    ...
    Following BEAT2 Face-Only V3 format: assistant-only, no system prompt, motion supervision with unified modality.
    ...
    # Add special tokens for face modality only (Face-Only V3)
    ...
    for conv_idx, (conv_id, turns) in enumerate(conversations.items()):
        ...
        for turn_idx, asst_turn in enumerate(asst_turns):
            ...
            # Process motion tokens (Face-Only V3)
            ...
            # Process face tokens only (Face-Only V3)
            for face_token in face_tokens:
                        if isinstance(face_token, np.ndarray):
                            face_token_val = face_token.tolist() if hasattr(face_token, 'tolist') else face_token
                        else:
                            face_token_val = face_token
                        face_token_str = safe_token_to_string(face_token_val, "face")
                        face_token_ids = tokenizer(face_token_str, add_special_tokens=False)["input_ids"]
                        motion_token_ids.extend(face_token_ids)
                        motion_labels.extend(face_token_ids)  # Face tokens are supervised
                        motion_modality_0.extend([False] * len(face_token_ids))  # Face is not text
                        motion_modality_1.extend([False] * len(face_token_ids))  # Face is not audio
                        motion_modality_2.extend([True] * len(face_token_ids))   # Face is modality 2 (motion)
...
def main():
    ...
    parser.add_argument("--face_dir", type=str, default="TOKENS_AGENT_25/face", 
    ...
    logging.info("Starting preprocessing of BEAT2 dataset (A-only version)")
    ...
    video_ids = []
    if not os.path.isdir(audio_dir) or not os.path.isdir(face_dir) or not os.path.isdir(textgrid_dir):
        logging.error("One or more required directories do not exist: audio_dir, face_dir, textgrid_dir")
        return
    ...
    for audio_path in Path(audio_dir).glob("*.npy"):
        ...
        textgrid_file = os.path.join(textgrid_dir, f"{video_id}.TextGrid")
        if os.path.exists(face_file) and os.path.exists(textgrid_file):
            video_ids.append(video_id)
        else:
            logging.debug(f"Skipping {video_id}, missing face tokens or TextGrid")
    ...
    for video_id in tqdm(video_ids, desc="Processing videos"):
        ...
        # Interleave into BEAT2 Face-Only V3 groups (text + 26 audio + 1 begin_of_motion + 52 face motion tokens per group)
    ...
    if all_turns:
        dataset = convert_to_huggingface_dataset(
            ...
            audio_segment_size=26,
            face_segment_size=52,  # 2.0 x 26 = 52, Face-Only V3
