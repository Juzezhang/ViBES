...
from multimodal_tokenizers.data.transforms.joints2rots.smplifyx import SMPLifyX3D
...
from scripts.plys2npy import plys2npy_smplx
import random
...
init_mean_shape = torch.zeros(opt.batchSize,300).to(device)
...
pred_betas = torch.zeros(opt.batchSize, 300).to(device)
...
for path in paths:
    ...
    for idx in range(0, num_seqs, opt.batchSize):
        ...
        #     return_verts=True,
        # )
        outputp = smplxmodel(betas=new_opt_betas,
                                transl= new_opt_cam_t[:,0],
                                expression=torch.zeros(new_opt_betas.shape[0], 100).to(device),
                                jaw_pose=torch.zeros(new_opt_betas.shape[0], 3).to(device),
                                global_orient=new_opt_pose[:, :3],
                                body_pose=new_opt_pose[:, 3 :21 * 3 + 3],
                                left_hand_pose=torch.zeros(new_opt_betas.shape[0], 15 * 3).to(device),
                                right_hand_pose=torch.zeros(new_opt_betas.shape[0], 15 * 3).to(device),
                                leye_pose=torch.zeros(new_opt_betas.shape[0], 3).to(device),
                                reye_pose=torch.zeros(new_opt_betas.shape[0], 3).to(device),
                                )
        ...
        for idx_save in range(len(batch_data)):
            mesh_p = trimesh.Trimesh(vertices=outputp.vertices[idx_save].detach().cpu().numpy().squeeze(), faces=smplxmodel.faces, process=False)
    ...
    plys2npy_smplx(dir_save, os.path.join(opt.save_folder))