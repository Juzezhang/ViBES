...
def read_speaking_tracks(root: Path) -> list[str]:
    texts: list[str] = []
    # First collect all paths
    paths = list(root.rglob('*_speaking_track.txt'))
    print(f"Found {len(paths)} speaking track files")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Generate word cloud from *_speaking_track.txt files')
    parser.add_argument('--root', required=True, help='Root directory containing transcript folders')
    parser.add_argument('--output', required=False, default='output/wordcloud_youtube_talking.png', help='Output PNG path')
...
def main():
    ...
    texts = read_speaking_tracks(root)
    if not texts:
        print('No *_speaking_track.txt files found.')
        sys.exit(2)
