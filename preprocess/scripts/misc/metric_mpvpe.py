...
# for gt_file in tqdm(gt_files, total=len(gt_files)):
    ...
    batch_size = len(gt_data['poses'])
    # b = len(res_data['poses'])
    # l = min(b, b)
    gt_poses_tensor = torch.tensor(gt_data['poses'], dtype=torch.float32).cuda()
    res_poses_tensor = torch.tensor(res_data['poses'], dtype=torch.float32).cuda()

    output_gt = smplx.forward(
        # betas=torch.tensor(gt_data['betas'], dtype=torch.float32).unsqueeze(0).repeat(batch_size, 1).cuda(),
        transl=torch.tensor(gt_data['trans'], dtype=torch.float32).cuda(),
        global_orient=gt_poses_tensor[:,:3],
        body_pose=gt_poses_tensor[:, 3:21 * 3 + 3],
        jaw_pose=gt_poses_tensor[:, 66:69],
        leye_pose=gt_poses_tensor[:, 69:72],
        reye_pose=gt_poses_tensor[:, 72:75],
        left_hand_pose=gt_poses_tensor[:, 25 * 3:40 * 3],
        right_hand_pose=gt_poses_tensor[:, 40 * 3:55 * 3],
        expression=torch.tensor(gt_data['expressions'], dtype=torch.float32).cuda(),
        return_verts=True,
        return_joints=True,
    )
    output_res = smplx.forward(
        # betas=torch.tensor(gt_data['betas'], dtype=torch.float32).unsqueeze(0).repeat(batch_size, 1).cuda(),
        transl=torch.tensor(res_data['trans'][:batch_size,:3], dtype=torch.float32).cuda(),
        global_orient=res_poses_tensor[:batch_size,:3],
        body_pose=res_poses_tensor[:batch_size, 3:21 * 3 + 3],
        jaw_pose=res_poses_tensor[:batch_size, 66:69],
        leye_pose=res_poses_tensor[:batch_size, 69:72],
        reye_pose=res_poses_tensor[:batch_size, 72:75],
        left_hand_pose=res_poses_tensor[:batch_size, 25 * 3:40 * 3],
        right_hand_pose=res_poses_tensor[:batch_size, 40 * 3:55 * 3],
        expression=torch.tensor(gt_data['expressions'][:batch_size,:], dtype=torch.float32).cuda(),
        return_verts=True,
        return_joints=True,
    )
    ...
    # calculate MPJPE
    mpjpe = calc_mpjpe(output_res.joints * 1000, output_gt.joints * 1000).mean().item()
    mpjpe_list.append(mpjpe)

    mpvpe = calc_mpjpe(output_res.vertices * 1000, output_gt.vertices * 1000).mean().item()
    mpvpe_list.append(mpvpe)

    # calculate PA-MPJPE
    pampjpe = calc_pampjpe(output_res.joints * 1000, output_gt.joints * 1000).mean().item()
...
print("ACCEL average：", np.mean(accel_list))