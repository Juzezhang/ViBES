...
def parse_arguments():
    ...
    parser.add_argument("--index_fps", type=int, required=False, default=20,
...
def process_and_align_motion_sequence(motion_file_path, output_file_path, output_mirrored_path,
    ...
    Args:
        ...
        index_fps (int): Frame rate used by the index file for start/end frames
    ...
    # Adjust frame indices from index FPS (HumanML3D uses 20 FPS) to the downsampled FPS.
    ...
    if 'humanact12' not in motion_file_str:
        ...
        # Extract the specified frame range with bounds checking
...
def main():
    ...
    processed_data_directory = pathlib.Path(os.path.join(args.dataset_path_processed, 'amass_data_align_25'))
