...
organizing audio and motion tokens (upper, lower_genmo) into fixed-size chunks and saving in
HuggingFace datasets format. Body only version without face or hand tokens.
...
- Lower body (genmo) tokens under data_root/TOKENS_AGENT_25/lower_genmo
...
- Unifies upper and lower_genmo tokens into a single motion modality (modality 2)
- Audio FPS: 12.5, Motion FPS: 12.5 (6.25*2) for upper + lower_genmo
- NO hand tokens: only upper + lower_genmo
- Group size: question (text OR audio) + answer text (with embedded audio) + 1 begin_of_motion + 26 interleaved motion tokens per group
- Token breakdown: 1 begin_of_motion + 26 motion tokens (13 upper + 13 lower_genmo in 1:1 alternating pattern)
- 3 modalities: text(0), audio(1), motion(2)
- Different system prompts for text vs audio input

Usage:
    python preprocess_hf_amass_dataset_body_upper_lower_genmo.py \
        --data_root /path/to/AMASS \
        --output_path ./processed_amass_body_upper_lower_genmo_train \
        --split train
"""
...
def initialize_token_sets(tokenizer):
    ...
    """
    global UPPER_TOKEN_IDS, LOWER_TOKEN_IDS
    ...
    logging.info(f"Initialized token sets: {len(UPPER_TOKEN_IDS)} upper, {len(LOWER_TOKEN_IDS)} lower tokens")
...
def calculate_position_encoding_indices(modality_masks, modality_fps=None):
    ...
    Args:
        ...
        modality_fps: Dictionary mapping modality index to fps value
                     Default: {1: 12.5, 2: 12.5} (upper + lower_genmo, motion=12.5fps=6.25*2)
...
def safe_token_to_string(token, modality):
    ...
    Args:
        ...
        modality: Type of token ("audio", "upper", or "lower")
...
def process_full_video(sequence_id, transcripts_question_dir, transcripts_answer_dir, audio_question_dir, upper_dir, lower_dir,
                       audio_fps=12.5, upper_fps=6.25, lower_fps=6.25):
    """
    ...
    Args:
        ...
        lower_dir: Directory containing lower body (genmo) tokens
        upper_fps: Upper body token frame rate
        lower_fps: Lower body (genmo) token frame rate

    Returns:
        List of processed segments with text, audio, upper body, and lower body (genmo) tokens
    """
    processed_segments = []
    ...
    if not os.path.exists(lower_token_file):
        logging.warning(f"No lower body (genmo) tokens found for {sequence_id}, skipping")
        return []
    ...
    # No face or hand tokens in upper + lower_genmo version

    # Load upper and lower (genmo) body tokens
    try:
        ...
        # Calculate required number of lower (genmo) tokens based on duration and lower FPS
        ...
        if len(lower_data) < target_lower_length:
            ...
            logging.info(f"Extended lower (genmo) tokens from {len(lower_data)} to {target_lower_length} by repeating (duration: {target_duration:.2f}s)")
        elif len(lower_data) > target_lower_length:
            # Truncate to match target duration
            lower_data_extended = lower_data[:target_lower_length]
            logging.info(f"Truncated lower (genmo) tokens from {len(lower_data)} to {target_lower_length} (duration: {target_duration:.2f}s)")
        else:
            lower_data_extended = lower_data
            logging.info(f"Lower (genmo) tokens already match target duration: {len(lower_data)} tokens for {target_duration:.2f}s")
        ...
        logging.info(f"Final lower (genmo) body tokens: {len(lower_data_extended)}, duration: {len(lower_data_extended) / lower_fps:.2f}s")

    except Exception as e:
        logging.error(f"Error loading upper/lower body tokens for {sequence_id}: {e}")
        return []
...
def convert_to_huggingface_dataset(
    ...
    Upper + lower_genmo version: upper and lower_genmo tokens in 1:1 interleaved pattern.
    ...
    Args:
        ...
        lower_segment_size: Number of lower body (genmo) tokens per group (default: 13)
        limit_sequences: Limit number of sequences to process
        split: Dataset split name
    Returns:
        Dataset: The created HuggingFace dataset
    """
    logging.info(f"Converting to Hugging Face dataset (upper + lower_genmo version) with tokenizer: {tokenizer.__class__.__name__}")
    ...
    for conv_idx, (conv_id, turns) in enumerate(conversations.items()):
        ...
        if len(turns) > 0:
            ...
            full_lower_tokens = first_turn.get("lower_tokens", [])  # Full lower (genmo) token array
            ...
            # Each audio group (26 tokens) corresponds to 13 upper + 13 lower_genmo = 26 motion tokens
            # So: required_audio_groups = len(full_upper_tokens) / 13 (use upper as reference)
            ...
            if len(all_audio_groups) > required_audio_groups:
                ...
                logging.info(f"Truncated audio groups from {len(all_audio_groups)} to {required_audio_groups} to match motion tokens length ({len(full_upper_tokens)} upper tokens)")
            else:
                # Use all available audio groups if not enough
                audio_groups = all_audio_groups
                if len(audio_groups) < required_audio_groups:
                    logging.warning(f"Only {len(audio_groups)} audio groups available, but {required_audio_groups} needed for {len(full_upper_tokens)} upper tokens")
            ...
            adjusted_lower_tokens = full_lower_tokens
            ...
            lower_has_real_data = first_turn.get("has_lower_file", False)
            ...
            for group_idx, group in enumerate(audio_groups):
                ...
                lower_tokens_chunk = adjusted_lower_tokens[group_idx*13:(group_idx+1)*13] if group_idx*13 < len(adjusted_lower_tokens) else []
                if len(lower_tokens_chunk) < 13:
                    lower_tokens_chunk.extend([0] * (13 - len(lower_tokens_chunk)))

                # Create unified motion token sequence with 1:1 interleaved pattern (upper + lower_genmo)
                motion_token_ids = []
                motion_labels = []
                motion_modality_0 = []
                motion_modality_1 = []
                ...
                # Process tokens in 1:1 interleaved pattern (upper + lower_genmo)
                # Pattern: upper_0, lower_0, upper_1, lower_1, ...
                upper_idx = 0
                lower_idx = 0

                while upper_idx < len(upper_tokens_chunk) or lower_idx < len(lower_tokens_chunk):
                    # Add 1 upper token
                    if upper_idx < len(upper_tokens_chunk):
                        ...
                        motion_labels.extend(upper_token_ids if upper_has_real_data else [-100] * len(upper_token_ids))  # Upper tokens supervised if file exists
                    ...
                    if lower_idx < len(lower_tokens_chunk):
                        ...
                        motion_labels.extend(lower_token_ids if lower_has_real_data else [-100] * len(lower_token_ids))  # Lower tokens supervised if file exists
    ...
    metadata = {
        ...
        "format_version": "3.0",
        "format_type": "user_assistant_with_system_and_unified_motion_modality_upper_lower_genmo",
        "text_format": "question (text OR audio) + answer text (with embedded audio) + 1 begin_of_motion + 26 interleaved motion tokens (13 upper + 13 lower_genmo in 1:1 alternating pattern) with unified motion supervision and precomputed position encoding indices",
        "position_encoding": "precomputed_indices_based_on_modality_fps",
        "modality_fps": {"1": 12.5, "2": 12.5},
        "supervision": "unified motion tokens (upper, lower_genmo supervised if available)",
        "tokenized": True,
        ...
        "modality_supervision": "unified_motion_upper_lower_genmo",
        "source": "AMASS_question_answer_v3_upper_lower_genmo"
    }
...
def main():
    """Main function to preprocess AMASS dataset with question-answer format and upper + lower_genmo motion tokens."""
    parser = argparse.ArgumentParser(description="Preprocess AMASS dataset with question-answer format (upper + lower_genmo)")
    ...
    parser.add_argument("--lower_dir", type=str, default="TOKENS_AGENT_25/lower_genmo",
                       help="Directory containing lower body (genmo) token files (relative to data_root)")
    ...
    parser.add_argument("--lower_fps", type=float, default=6.25,
                       help="Lower body (genmo) tokens per second")
    ...
    logging.info("Starting preprocessing of AMASS dataset (upper + lower_genmo version)")
    ...
    sequence_ids = []
    if not os.path.isdir(transcripts_answer_dir) or not os.path.isdir(transcripts_question_dir) or not os.path.isdir(audio_question_dir) or not os.path.isdir(upper_dir) or not os.path.isdir(lower_dir):
        logging.error("One or more required directories do not exist: transcripts_answer_dir, transcripts_question_dir, audio_question_dir, upper_dir, lower_dir")
        return
    ...
    for transcripts_answer_path in Path(transcripts_answer_dir).glob("*.txt"):
        ...
        transcripts_question_file = os.path.join(transcripts_question_dir, f"{sequence_id}.txt")
        audio_question_file = os.path.join(audio_question_dir, f"{sequence_id}.npy")
        if os.path.exists(upper_file) and os.path.exists(lower_file) and os.path.exists(transcripts_question_file) and os.path.exists(audio_question_file):
            sequence_ids.append(sequence_id)
        else:
            logging.debug(f"Skipping {sequence_id}, missing required files (upper, lower_genmo, transcripts_question, or audio_question)")
    ...
    for sequence_id in tqdm(sequence_ids, desc="Processing videos"):
        ...
        processed_segments = process_full_video(
            sequence_id, transcripts_question_dir, transcripts_answer_dir, audio_question_dir, upper_dir, lower_dir, args.audio_fps, args.upper_fps, args.lower_fps
        )
        ...
        for seg in processed_segments:
            ...
            all_turns.append({
                ...
                "lower_tokens": seg["lower_tokens"],  # Full lower (genmo) token array
