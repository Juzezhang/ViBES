...
"""
Preprocess BEAT2 dataset into A-only sequences (assistant-only) for fullbody_genmo version,
organizing audio and fullbody_genmo tokens into fixed-size chunks with unified motion modality.
Fullbody GENMO version with single body part.
...
Key features for fullbody_genmo version:
...
- Processes fullbody_genmo motion tokens mapped to upper token vocabulary (<|upper_X|>)
- Audio FPS: 12.5, Motion FPS: 6.25 (6.25*1) for single fullbody part
- Group size: text + 26 audio + 1 begin_of_motion + 13 fullbody_genmo motion tokens per group
- Token breakdown: 1 begin_of_motion + 13 motion tokens (fullbody_genmo mapped as upper)
- 3 modalities: text(0), audio(1), motion(2)

Usage:
    python preprocess_hf_beat2_dataset_body_fullbody_genmo.py \
        --data_root /path/to/BEAT2/beat_english_v2.0.0 \
        --output_path ./processed_beat2_fullbody_genmo_train \
        --split train
"""
...
def process_full_video(video_id, transcript_text, audio_dir, fullbody_genmo_dir,
    ...
    fullbody_genmo_token_file = os.path.join(fullbody_genmo_dir, f"{video_id}.npy")
    ...
    if not os.path.exists(fullbody_genmo_token_file):
        logging.warning(f"No fullbody_genmo tokens found for {video_id}, skipping")
        return []
    ...
    try:
        ...
        fullbody_genmo_tokens_ts = [(fullbody_genmo_data[i], i / fullbody_genmo_fps) for i in range(len(fullbody_genmo_data))]
        logging.info(f"Loaded {len(fullbody_genmo_data)} fullbody_genmo tokens for {video_id}, duration: {len(fullbody_genmo_data) / fullbody_genmo_fps:.2f}s")
    except Exception as e:
        logging.error(f"Error loading fullbody_genmo tokens for {video_id}: {e}")
        return []
...
def interleave_groups_from_full_segment(segment, audio_group_size: int = 26, upper_group_size: int = 13, audio_fps: float = 12.5):
    """
    From one full A-only segment, build interleaved groups following BEAT2 format:
    per group: text (words in time window), 26 audio tokens, 1 begin_of_motion token, 13 fullbody_genmo motion tokens (mapped as upper).
    """
    audio_tokens = segment["audio_tokens"]
    upper_tokens = segment["upper_tokens"]  # fullbody_genmo tokens mapped as upper
    text = segment["text"]
    ...
    for i in range(total_groups):
        ...
        groups.append({
            ...
            "upper_tokens": chunk_upper,  # fullbody_genmo mapped as upper
        })
    return groups

def process_chunk_to_record(chunk, conv_id, tokenized_records):
    """
    Process a chunk of turns into a tokenized record with fullbody_genmo motion support and position encoding indices.
...
def convert_to_huggingface_dataset(
    ...
    """
    Convert interleaved turns into a HuggingFace dataset with tokenized format and fullbody_genmo motion support.
    Following BEAT2 format: assistant-only, no system prompt, fullbody_genmo motion supervision.
    ...
    """
    logging.info(f"Converting to Hugging Face dataset (fullbody_genmo version) with tokenizer: {tokenizer_name}")
    ...
    for conv_idx, (conv_id, turns) in enumerate(conversations.items()):
        ...
        for turn_idx, asst_turn in enumerate(asst_turns):
            ...
            # Process fullbody_genmo motion tokens (mapped as upper) - sequential, no interleaving
            upper_tokens_list = asst_turn.get("upper_tokens", [])  # fullbody_genmo mapped as upper
            ...
            for upper_idx in range(len(upper_tokens_list)):
                upper_token = upper_tokens_list[upper_idx]
                ...
                motion_labels.extend(upper_token_ids)  # Fullbody_genmo tokens are supervised
            ...
            # Compose the full turn with motion tokens
    ...
    metadata = {
        ...
        "format_version": "3.0",
        "format_type": "assistant_only_fullbody_genmo_with_position_encoding",
        "text_format": "text+audio+fullbody_genmo_motion_tokens (mapped as upper) with motion supervision and precomputed position encoding indices",
        "position_encoding": "precomputed_indices_based_on_modality_fps",
        "modality_fps": {"1": 12.5, "2": 6.25},
        "supervision": "fullbody_genmo tokens (mapped as upper, supervised)",
        "tokenized": True,
        ...
        "source": "BEAT2_A_only_fullbody_genmo"
    }
...
def main():
    """Main function to preprocess BEAT2 dataset (A-only version) with fullbody_genmo motion support."""
    parser = argparse.ArgumentParser(description="Preprocess BEAT2 dataset (A-only) with fullbody_genmo motion support")
    ...
    logging.info("Starting preprocessing of BEAT2 dataset (A-only fullbody_genmo version)")
    ...
    video_ids = []
    if not os.path.isdir(audio_dir) or not os.path.isdir(fullbody_genmo_dir) or not os.path.isdir(textgrid_dir):
        logging.error("One or more required directories do not exist: audio_dir, fullbody_genmo_dir, textgrid_dir")
        return
    ...
    for audio_path in Path(audio_dir).glob("*.npy"):
        ...
        fullbody_genmo_file = os.path.join(fullbody_genmo_dir, f"{video_id}.npy")
        textgrid_file = os.path.join(textgrid_dir, f"{video_id}.TextGrid")
        if os.path.exists(fullbody_genmo_file) and os.path.exists(textgrid_file):
            video_ids.append(video_id)
        else:
            logging.debug(f"Skipping {video_id}, missing fullbody_genmo tokens or TextGrid")
    ...
    for video_id in tqdm(video_ids, desc="Processing videos"):
        ...
        processed_segments = process_full_video(
            video_id, transcript_text, audio_dir, fullbody_genmo_dir, args.audio_fps, args.fullbody_genmo_fps
        )
        ...
        # Interleave into BEAT2-style groups (text + 26 audio + 1 begin_of_motion + 13 fullbody_genmo motion tokens per group)
        for seg in processed_segments:
            groups = interleave_groups_from_full_segment(seg, audio_group_size=26, upper_group_size=13, audio_fps=args.audio_fps)
            for idx, g in enumerate(groups):
                all_turns.append({
                    ...
                    "upper_tokens": g["upper_tokens"],  # fullbody_genmo mapped as upper
    ...
    if all_turns:
        dataset = convert_to_huggingface_dataset(
            ...
            audio_segment_size=26,
            upper_segment_size=13, ### 0.5 x 26 = 13, fullbody_genmo version
