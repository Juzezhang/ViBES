...
- Processes upper and lower_genmo motion tokens with unified modality (no face, no hand)
- Audio FPS: 12.5, Motion FPS: 12.5 (6.25*2) for upper + lower_genmo
- Group size: text + 26 audio + 1 begin_of_motion + 26 interleaved motion tokens per group
...
Usage:
    python preprocess_hf_beat2_dataset_body_upper_lower_genmo.py \
        --data_root /path/to/BEAT2/beat_english_v2.0.0 \
        --output_path ./processed_beat2_upper_lower_genmo_train \
        --split train
"""
...
def calculate_position_encoding_indices(modality_masks, modality_fps=None):
    ...
    Args:
        ...
        modality_fps: Dictionary mapping modality index to fps value
                     Default: {1: 12.5, 2: 12.5} (upper + lower_genmo version, motion=12.5fps=6.25*2)
...
def process_full_video(video_id, transcript_text, audio_dir, upper_dir, lower_dir,
    ...
    Returns:
        List of processed segments with text, audio, upper, and lower body (genmo) tokens
    """
    processed_segments = []
    ...
    if not os.path.exists(lower_token_file):
        logging.warning(f"No lower body (genmo) tokens found for {video_id}, skipping")
        return []
    ...
    try:
        ...
        logging.info(f"Loaded {len(lower_data)} lower (genmo) body tokens for {video_id}, duration: {len(lower_data) / lower_fps:.2f}s")
    except Exception as e:
        logging.error(f"Error loading upper/lower body tokens for {video_id}: {e}")
        return []
...
def interleave_groups_from_full_segment(segment, audio_group_size: int = 26, upper_group_size: int = 13, lower_group_size: int = 13, audio_fps: float = 12.5):
    """
    From one full A-only segment, build interleaved groups following BEAT2 format:
    per group: text (words in time window), 26 audio tokens, 1 begin_of_motion token, 26 interleaved motion tokens (13 upper + 13 lower_genmo in 1:1 alternating pattern).
    """
...
def process_chunk_to_record(chunk, conv_id, tokenized_records):
    ...
    modality_masks = [
        ...
        chunk["modality_masks_2"],  # Motion tokens (modality 2) - unified upper, lower_genmo
    ]
    ...
    tokenized_record = {
        ...
        "modality_masks_2": chunk["modality_masks_2"],  # True for motion tokens (unified upper, lower_genmo)
...
def convert_to_huggingface_dataset(
    ...
    Args:
        ...
        lower_segment_size: Number of lower body (genmo) tokens per group (default: 13, used in interleaved pattern)
    Returns:
        Dataset: The created HuggingFace dataset
    """
    logging.info(f"Converting to Hugging Face dataset (upper + lower_genmo version) with tokenizer: {tokenizer_name}")
    ...
    for conv_idx, (conv_id, turns) in enumerate(conversations.items()):
        ...
        for turn_idx, asst_turn in enumerate(asst_turns):
            ...
            # Process motion tokens in 1:1 interleaved pattern: upper, lower_genmo, upper, lower_genmo...
            ...
            if upper_tokens or lower_tokens:
            ...
            # Process tokens in 1:1 interleaved pattern
            # Pattern: upper_0, lower_0, upper_1, lower_1, ...
            upper_idx = 0
            lower_idx = 0

            while upper_idx < len(upper_tokens) or lower_idx < len(lower_tokens):
                ...
                if lower_idx < len(lower_tokens):
                    ...
                    motion_labels.extend(lower_token_ids)  # Lower (genmo) tokens are supervised
  ...
  # EOS is not upper body
    ...
    metadata = {
        ...
        "format_version": "3.0",
        "format_type": "assistant_only_mot_with_position_encoding_upper_lower_genmo",
        "text_format": "text+audio+interleaved_motion_tokens (upper,lower_genmo in 1:1 alternating pattern) with motion supervision and precomputed position encoding indices",
        "position_encoding": "precomputed_indices_based_on_modality_fps",
        "modality_fps": {"1": 12.5, "2": 12.5},
        "supervision": "upper, lower_genmo body tokens",
        "tokenized": True,
        "max_seq_length": max_seq_length,
        "assistant_prefix": assistant_prefix.strip(),
        "modality_masks": "masks_0 for text, masks_1 for audio, masks_2 for motion (unified upper, lower_genmo)",
        "modality_supervision": "upper_lower_genmo",
        "source": "BEAT2_A_only_upper_lower_genmo"
    }
...
def main():
    """Main function to preprocess BEAT2 dataset (A-only version) with upper + lower_genmo motion tokens."""
    parser = argparse.ArgumentParser(description="Preprocess BEAT2 dataset (A-only) with upper + lower_genmo")
    ...
    logging.info("Starting preprocessing of BEAT2 dataset (A-only upper + lower_genmo version)")
    ...
    video_ids = []
    if not os.path.isdir(audio_dir) or not os.path.isdir(upper_dir) or not os.path.isdir(lower_dir) or not os.path.isdir(textgrid_dir):
        logging.error("One or more required directories do not exist: audio_dir, upper_dir, lower_dir, textgrid_dir")
        return
    ...
    for audio_path in Path(audio_dir).glob("*.npy"):
        ...
        textgrid_file = os.path.join(textgrid_dir, f"{video_id}.TextGrid")
        if os.path.exists(upper_file) and os.path.exists(lower_file) and os.path.exists(textgrid_file):
            video_ids.append(video_id)
        else:
            logging.debug(f"Skipping {video_id}, missing upper body tokens or lower (genmo) body tokens or TextGrid")
    ...
    for video_id in tqdm(video_ids, desc="Processing videos"):
        ...
        processed_segments = process_full_video(
            video_id, transcript_text, audio_dir, upper_dir, lower_dir, args.audio_fps, args.upper_fps, args.lower_fps
        )
        ...
        # Interleave into BEAT2-style groups (text + 26 audio + 1 begin_of_motion + 26 interleaved motion tokens per group)
        for seg in processed_segments:
            groups = interleave_groups_from_full_segment(seg, audio_group_size=26, upper_group_size=13, lower_group_size=13, audio_fps=args.audio_fps)
    ...
    if all_turns:
        dataset = convert_to_huggingface_dataset(
            ...
            audio_segment_size=26,
            upper_segment_size=13, ### 0.5 x 26 = 13, upper + lower_genmo version
            lower_segment_size=13, ### 0.5 x 26 = 13, upper + lower_genmo version
