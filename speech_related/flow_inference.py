...
def fade_in_out(fade_in_mel, fade_out_mel, window):
    ...
    # Ensure window is a torch.Tensor
    if not isinstance(window, torch.Tensor):
        window = torch.tensor(window, dtype=fade_in_mel.dtype)
    
    # Make sure window is on the same device as mel spectrograms
    window = window.to(fade_in_mel.device)
    
    mel_overlap_len = int(window.shape[0] / 2)
    
    # Break down the complex operation to ensure type consistency
    fade_in_part = fade_in_mel[..., :mel_overlap_len] * window[:mel_overlap_len]
    fade_out_part = fade_out_mel[..., -mel_overlap_len:] * window[mel_overlap_len:]
    
    # Perform addition operation
    fade_in_mel[..., :mel_overlap_len] = fade_in_part + fade_out_part
