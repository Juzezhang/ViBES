#!/usr/bin/env python3
"""
Combined inference script for Face + Body motion generation.

Generates both facial expressions and body motion from text prompts using
two separate models (face expert and body expert). Produces a single MP4
video rendered from SMPLX2020 with combined face and body parameters.

Pipeline:
  1. Load face model -> generate audio + face tokens from text
  2. Load body model -> generate body tokens (upper/lower/hand) from text
  3. Decode audio tokens -> waveform
  4. Decode face tokens -> jaw pose, expressions
  5. Decode body tokens -> upper/lower body poses + global translation
  6. Merge face features (jaw, expressions) into full SMPLX body
  7. Render full body video (SMPLX, GENMO-style) with synchronized audio

Usage:
    python inference/inference_a2m_face_body.py \\
        --checkpoint_face ./ViBES-Face \\
        --checkpoint_body /path/to/body_checkpoint \\
...
FACE_JAW_POSE_START = 6
FACE_JAW_POSE_END = 12
FACE_EXPRESSION_START = 12

# Body feature dimensions
...
VAE_CHECKPOINT_GLOBAL = os.environ.get(
    ...
    os.path.join(ROOT_DIR, 'experiments', 'multimodal_tokenizer',
...
def run_generation(model, tokenizer, device, user_text, body_part,
    ...
    # Apply token offset to motion tokens
...
def reconstruct_body_pose_with_face(rec_upper, rec_lower, rec_pose_jaw_6d,
                                    batch_size, num_frames, device):
    """
    Reconstruct full body pose from body parts, using jaw from face model.

    Unlike reconstruct_full_body_pose(), this uses the actual jaw rotation
    from the face model instead of zeroing it out.
    """
    ...
    # Hands: zeros (hand VAE output not yet used for pose reconstruction)
    ...
    # Jaw: from face model (actual rotation, not zeros)
    jaw_aa = rotation_6d_to_axis_angle(
        rec_pose_jaw_6d.reshape(batch_size * num_frames, 6)
    ).reshape(batch_size * num_frames, 3)

    # Combine all parts
    rec_pose = pose_upper_full + pose_lower_full + pose_hands_full
    rec_pose[:, POSE_JAW[0]:POSE_JAW[1]] = jaw_aa
...
def generate_face_body_from_text(
    model_face,
    model_body,
    tokenizer,
    device,
    ...
    max_new_tokens=1024,
    temperature_face=0.2,
    top_p_face=0.8,
    temperature_body=0.1,
    top_p_body=0.1,
    cam_beta=2.5,
    fixed_camera=False,
    front_view=False,
    ...
    """
    Generate synchronized face and body motion from text.

    Returns:
        True if generation succeeded
    """
    print("\n=== Loading VAE Models ===")
    # Face VAE (for decoding face tokens)
    vae_face_model = VQVAEConvZeroDSUS1_PaperVersion(
        vae_layer=3, code_num=512, codebook_size=512,
        vae_quantizer_lambda=1, vae_test_dim=FACE_FEATURE_DIM
    )
    face_ckpt = torch.load(VAE_CHECKPOINT_FACE, map_location="cpu", weights_only=False)
    state_dict_face = {}
    for key, value in face_ckpt.items():
        if 'vae_face' in key:
            state_dict_face[key.replace('vae_face.', '')] = value
    vae_face_model.load_state_dict(state_dict_face, strict=True)
    vae_face_model.eval().to(device)

    # Body VAEs (upper, lower, hand, global)
    smplx_2020 = load_smplx_model(SMPLX_MODEL_DIR, device)
    _, vae_upper, vae_lower, vae_global, vae_hand = load_vae_models(
        device, VAE_CHECKPOINT_MAIN, VAE_CHECKPOINT_FACE, VAE_CHECKPOINT_GLOBAL
    )

    # ========================================================================
    # Step 1: Generate face tokens + audio tokens
    # ========================================================================
    print("\n=== Generating Face Tokens ===")
    face_modality_tokens = run_generation(
        model_face, tokenizer, device, user_text, body_part="face",
        max_new_tokens=max_new_tokens, temperature=temperature_face, top_p=top_p_face,
    )
    audio_tokens = face_modality_tokens['audio']
    face_tokens = face_modality_tokens['face']
    print(f"  audio tokens: {len(audio_tokens)}, face tokens: {len(face_tokens)}")

    # ========================================================================
    # Step 2: Generate body tokens
    # ========================================================================
    print("\n=== Generating Body Tokens ===")
    body_modality_tokens = run_generation(
        model_body, tokenizer, device, user_text, body_part="body",
        max_new_tokens=max_new_tokens, temperature=temperature_body, top_p=top_p_body,
    )
    upper_tokens = body_modality_tokens['upper']
    lower_tokens = body_modality_tokens['lower']
    hand_tokens = body_modality_tokens['hand']
    print(f"  upper tokens: {len(upper_tokens)}, lower tokens: {len(lower_tokens)}, hand tokens: {len(hand_tokens)}")

    # ========================================================================
    # Step 3: Decode audio (using audio from face model)
    # ========================================================================
    print("\n=== Decoding Audio ===")
    ...
    # ========================================================================
    # Step 4: Decode face tokens
    # ========================================================================
    print("\n=== Decoding Face Tokens ===")
    face_token_tensor = torch.tensor(face_tokens, device=device).unsqueeze(0)
    rec_face = vae_face_model.decode(face_token_tensor.int())  # (1, T_face, 112)

    rec_jaw_pose_6d = rec_face[0, :, FACE_JAW_POSE_START:FACE_JAW_POSE_END]
    rec_exp = rec_face[0, :, FACE_EXPRESSION_START:]
    n_face = rec_face.shape[1]

    # ========================================================================
    # Step 5: Decode body tokens
    # ========================================================================
    ...
    # Align sequence lengths across body parts
    n_body = min(rec_upper.shape[1], rec_lower.shape[1], rec_hand.shape[1])
    rec_upper = rec_upper[:, :n_body, :]
    rec_lower = rec_lower[:, :n_body, :LOWER_BODY_FEATURE_DIM]
    rec_hand = rec_hand[:, :n_body, :]

    # ========================================================================
    # Step 6: Merge face + body
    # ========================================================================
    print("\n=== Merging Face and Body ===")

    # Use the shorter of face and body sequences
    n = min(n_face, n_body)
    rec_upper = rec_upper[:, :n, :]
    rec_lower = rec_lower[:, :n, :]
    rec_exp_aligned = rec_exp[:n, :].unsqueeze(0)  # (1, n, 100)
    rec_jaw_6d_aligned = rec_jaw_pose_6d[:n, :].unsqueeze(0)  # (1, n, 6)

    bs = 1

    # Reconstruct full body pose with jaw from face model
    rec_pose, rec_lower2global = reconstruct_body_pose_with_face(
        rec_upper, rec_lower, rec_jaw_6d_aligned, bs, n, device
    )

    # ========================================================================
    # Step 7: Global motion (translation)
    # ========================================================================
    to_global = rec_lower
    ...
    rec_pose_6d = axis_angle_to_6d(
        rec_pose.reshape(n, SMPLX_NUM_JOINTS, SMPLX_DIMS_PER_JOINT)
    ).reshape(n, SMPLX_NUM_JOINTS * 6)
    rec_exps = rec_exp_aligned.to(device)
    rec_beta = torch.tile(rec_beta, (n, 1))

    # ========================================================================
    # Step 8: SMPLX mesh generation
    # ========================================================================
    ...
    joints_raw = vertices_rec.joints.detach()  # (n, J, 3)

    # Normalize using SMPLX joints directly
    verts = verts_smplx_raw.clone()
    offset = joints_raw[0, 0].clone()  # pelvis at frame 0
    offset[1] = verts[:, :, 1].min()
    verts = verts - offset
    joints_norm = joints_raw - offset
    T_rot = compute_T_ayfz2ay(joints_norm[[0]], inverse=True)
    verts = apply_T_on_points(verts, T_rot)
    root_points = apply_T_on_points(joints_norm, T_rot)[:, 0, :]  # pelvis (n, 3)
...
def main():
    parser = argparse.ArgumentParser(
        description='Combined Face + Body Motion Generation',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument('--checkpoint_face', type=str, default="./ViBES-Face",
                        help='Path to face model checkpoint directory')
    ...
    parser.add_argument('--temperature_face', type=float, default=0.2,
                        help='Sampling temperature for face model')
    parser.add_argument('--top_p_face', type=float, default=0.8,
                        help='Top-p sampling for face model')
    parser.add_argument('--temperature_body', type=float, default=0.1,
                        help='Sampling temperature for body model (must be > 0 when do_sample=True)')
    parser.add_argument('--top_p_body', type=float, default=0.1,
    ...
    # Validate paths
    for name, path in [("Face checkpoint", args.checkpoint_face), ("Body checkpoint", args.checkpoint_body)]:
    ...
    generate_face_body_from_text(
        model_face, model_body, tokenizer, device,
        user_text=args.user_text,
        output_dir=args.output_dir,
        output_prefix=args.output_prefix,
        max_new_tokens=args.max_new_tokens,
        temperature_face=args.temperature_face,
        top_p_face=args.top_p_face,
        temperature_body=args.temperature_body,
        top_p_body=args.top_p_body,
