#!/usr/bin/env python3
"""
Inference script for Audio-to-Motion FullBody GENMO generation.

This script generates full-body motion sequences from text prompts using a multimodal
generative model with a single GenmoFull VQ-VAE for the entire body (135D features).
It combines text-to-speech, motion token generation, and 3D pose reconstruction
to produce animated video outputs with synchronized audio.

Features:
- Text-to-motion generation with audio synthesis
- Single GenmoFull VQ-VAE decoding (no upper/lower/hand split)
- GENMO 145D -> SMPL-X reconstruction with local velocity integration
- SMPLX mesh generation and video rendering

Usage:
    # Recommended: Run from project root directory
    python -m inference.inference_body_fullbody_genmo \\
    ...
    cd inference
    python inference_body_fullbody_genmo.py \\
        --checkpoint <path_to_checkpoint> \\
        --user_text "Your text prompt here" \\
        --output_dir ./output
"""
...
# GENMO feature dimensions
GENMO_FULL_FEATURE_DIM = 135  # GenmoFull VAE output dimension (no betas)
GENMO_145_DIM = 145  # Full GENMO representation dimension (with zero betas)
GENMO_BODY_POSE_DIM = 126  # 21 joints * 6D rotation = 126
GENMO_NUM_BODY_JOINTS = 21  # Number of body joints in GENMO representation
GENMO_BETAS_DIM = 10  # Number of beta (shape) parameters in GENMO 145D
GENMO_GLOBAL_ORIENT_START = 136  # Start index of global orientation in 145D
GENMO_GLOBAL_ORIENT_END = 142  # End index of global orientation in 145D
GENMO_LOCAL_VEL_START = 142  # Start index of local velocity in 145D
GENMO_LOCAL_VEL_END = 145  # End index of local velocity in 145D
...
# VAE model path
VAE_CHECKPOINT_GENMO_FULL = os.path.join(
    ROOT_DIR, 'model_files', 'pretrained_cpt', 'VQVAE_0320_GenmoFull', 'last.ckpt'
)
...
def load_genmo_full_vae(device, checkpoint_path):
    """
    Load and initialize the GenmoFull VQ-VAE model.

    Args:
        device: Device to load model on (e.g., 'cuda' or 'cpu')
        checkpoint_path: Path to the GenmoFull VAE checkpoint file

    Returns:
        Initialized MotionGPTVQVaeAdapter in evaluation mode

    Raises:
        FileNotFoundError: If checkpoint file is not found
        RuntimeError: If model loading fails
    """
    print("Loading GenmoFull VQ-VAE model...")

    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Checkpoint file not found: {checkpoint_path}")

    vae_full = MotionGPTVQVaeAdapter(
        nfeats=135,
        code_dim=256,
        code_num=256,
        mu=0.99,
    ...
    # Extract keys with 'vae_body.' prefix
    prefix = "vae_body."
    extracted = {}
    ...
    if extracted:
        vae_full.load_state_dict(extracted, strict=True)
    else:
        # Fallback: try loading state_dict directly
        vae_full.load_state_dict(state_dict, strict=True)

    vae_full.eval()
    vae_full.to(device)

    print("GenmoFull VQ-VAE model loaded successfully!")
    return vae_full
...
def generate_motion_from_text(
    ...
    1. Loads GenmoFull VAE model for motion decoding
    2. Tokenizes input text and generates multimodal tokens
    3. Decodes audio tokens to waveform
    4. Decodes motion tokens to GENMO 135D, reconstructs GENMO 145D
    5. Converts GENMO 145D to SMPL-X pose via rotation conversions
    ...
    # Load GenmoFull VAE model
    vae_full = load_genmo_full_vae(device, VAE_CHECKPOINT_GENMO_FULL)
    ...
    # fullbody_genmo was mapped as <|upper_X|> during training
    ...
    # ========================================================================
    # Motion Token Decoding (Single GenmoFull VAE)
    # ========================================================================

    # Convert token list to tensor and decode using GenmoFull VAE
    upper_token_tensor = torch.tensor(upper_tokens, device=device).unsqueeze(0)

    # Decode: vae_full.decode(upper_tokens) -> (1, T, 135)
    rec = vae_full.decode(upper_token_tensor.int())  # (1, T, 135)

    T = rec.shape[1]

    # Insert zero betas to reconstruct GENMO 145D:
    # rec_145 = cat([rec[:,:126], zeros(T,10), rec[:,126:]]) -> 145D
    # rec[:, :, :126]  = body_pose_r6d (21 joints * 6)
    # zeros(1, T, 10)  = betas
    # rec[:, :, 126:]  = remaining 9 dims (global_orient_r6d[6] + local_vel[3])
    rec_145 = torch.cat([
        rec[:, :, :GENMO_BODY_POSE_DIM],
        torch.zeros(1, T, GENMO_BETAS_DIM, device=device, dtype=rec.dtype),
        rec[:, :, GENMO_BODY_POSE_DIM:],
    ], dim=2)  # (1, T, 145)

    # ========================================================================
    # GENMO 145D -> SMPL-X Reconstruction
    # ========================================================================

    # Squeeze batch dimension for per-frame processing
    rec_145 = rec_145[0]  # (T, 145)

    # Body pose: r6d -> rotation matrix -> axis angle
    body_pose_r6d = rec_145[:, :GENMO_BODY_POSE_DIM].reshape(T, GENMO_NUM_BODY_JOINTS, 6)
    body_pose_mat = rotation_6d_to_matrix(body_pose_r6d)  # (T, 21, 3, 3)
    body_pose_aa = matrix_to_axis_angle(body_pose_mat)  # (T, 21, 3)

    # Global orientation: r6d -> rotation matrix -> axis angle
    global_orient_r6d = rec_145[:, GENMO_GLOBAL_ORIENT_START:GENMO_GLOBAL_ORIENT_END].reshape(T, 1, 6)
    global_orient_mat = rotation_6d_to_matrix(global_orient_r6d)  # (T, 1, 3, 3)
    global_orient_aa = matrix_to_axis_angle(global_orient_mat).reshape(T, 3)  # (T, 3)

    # Local velocity -> world translation
    local_vel = rec_145[:, GENMO_LOCAL_VEL_START:GENMO_LOCAL_VEL_END]  # (T, 3)
    transl = integrate_local_velocity(local_vel, global_orient_aa)  # (T, 3)
    ...
    # Build full 55-joint pose tensor (axis-angle, 55*3=165)
    rec_pose = torch.zeros(n, SMPLX_NUM_JOINTS * SMPLX_NUM_DIMS_PER_JOINT, device=device)
    rec_pose[:, POSE_GLOBAL_ORIENT_INDICES[0]:POSE_GLOBAL_ORIENT_INDICES[1]] = global_orient_aa
    rec_pose[:, POSE_BODY_START_IDX:POSE_BODY_END_IDX] = body_pose_aa.reshape(n, -1)
    # Jaw, hands, eyes remain zero

    # Convert axis-angle to 6D rotation for consistency
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Audio-to-Motion FullBody GENMO Generation Inference',
